...
*/
LW('lw-list-item', class Item extends LW.ContentElement {
    // Item's properties.
    ...
    set _focused(value) {
        ...
        if (value) {
            ...
            if (that.ownerListBox && that.ownerListBox.ownerElement && LW.ComboBox &&
                that.ownerListBox.ownerElement instanceof LW.ComboBox) {
    ...
    /**
    * lw-list-item's template.
    */
    template() {
        return `<div id="container">
                   <span id='itemInput' class='lw-input lw-hidden'></span>
                    <span class="lw-overlay" id="overlay"></span>
                    <div id="itemContainer" class="lw-content lw-list-item-container"><span class="lw-content-label" inner-h-t-m-l=\'[[innerHTML]]\' role="presentation"><content></content></span><span inner-h-t-m-l=\'[[details]]\' id="details" class="lw-content-details"></span></div>
                </div>`;
    }
    ...
    appendChild(node) {
        ...
        if (!that.isCompleted || node.classList.contains('lw-ripple')) {
    ...
    /**
    * lw-list-item's event listeners.
    **/
    ...
    */
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        if (propertyName === 'hidden') {
            if (!newValue) {
                that.$.removeClass('lw-hidden');
            }
            else {
                that.$.addClass('lw-hidden');
            }
        }
    ...
    getListBox() {
        ...
        while (parent instanceof LW.ListBox === false && parent !== null) {
            parent = parent.parentNode;
        }

        if (!parent && LW.EnableShadowDOM) {
  ...
  */
    _itemOnMouseEnter() {
        ...
        if (LW.ListBox.DragDrop.Dragging && that.ownerListBox.allowDrop) {
            LW.ListBox.DragDrop.DropDetails = { item: that, position: 'bottom' };
        }
    ...
    */
    _itemOnMouseMove(event) {
        ...
        if (LW.ListBox.DragDrop.Dragging && that.ownerListBox.allowDrop && !LW.Utilities.Core.isMobile) {
            const itemsWithFeedback = [].slice.call(that.ownerListBox.getElementsByClassName('lw-list-item-bottom-line-feedback'));

            for (let i = 0; i < itemsWithFeedback.length; i++) {
                itemsWithFeedback[i].$.removeClass('lw-list-item-line-feedback');
                itemsWithFeedback[i].$.removeClass('lw-list-item-bottom-line-feedback');
            }

            LW.ListBox.DragDrop.DropDetails = { item: that, position: 'top' };
            ...
            that.$.removeClass('lw-list-item-line-feedback');
            that.$.removeClass('lw-list-item-bottom-line-feedback');
            ...
            if (visibleItems.indexOf(that) === visibleItems.length - 1 || that.parentNode.lastElementChild === that) {
                ...
                if (event.pageY - window.pageYOffset > rect.top + (rect.height / 2) - 1) {
                    that.$.addClass('lw-list-item-bottom-line-feedback');
                    LW.ListBox.DragDrop.DropDetails = { item: that, position: 'bottom' };
                }
                else if (!arguments[1]) {
                    that.$.addClass('lw-list-item-line-feedback');
                }
            }
            else if (!arguments[1]) {
                that.$.addClass('lw-list-item-line-feedback');
    ...
    */
    _itemOnMouseLeave() {
        ...
        if (LW.ListBox.DragDrop.Dragging) {
            that.$.removeClass('lw-list-item-line-feedback');
            that.$.removeClass('lw-list-item-bottom-line-feedback');
        }
    ...
    _setDisplayMode(value) {
        ...
        if (value === 'plain') {
            that.$itemInput.addClass('lw-hidden');
            return;
        }

        that.$itemInput.removeClass('lw-hidden');
    }
    ...
    _setItemColor() {
        const that = this,
            label = that.querySelector('.lw-content-label'),
...
*/
LW('lw-list-items-group', class ListItemsGroup extends LW.ContentElement {
    // Item's properties.
    static get properties() {
        return {
            'label': {
    ...
    /**
    * lw-list-items-group's template.
    */
    template() {
        return `<div id="container" role="presentation">
                 <span id="itemsLabel" class="lw-list-items-group-label">[[label]]</span>
                 <div id="listItemsGroupContainer" inner-h-t-m-l=\'[[innerHTML]]\' class="lw-list-items-group-container" role="presentation">
                    <content></content>
                 </div>
               </div>`;
    }
...
*/
LW('lw-list-box', class ListBox extends LW.ScrollViewer {
    ...
    */
    template() {
        return `<div id="container" class="lw-container" role="presentation">
                    <div class="lw-list-box-filter-input-container lw-hidden" id="filterInputContainer" role="presentation"><input id="filterInput" disabled="[[disabled]]" placeholder='[[filterInputPlaceholder]]' role="searchbox" aria-label="[[filterInputPlaceholder]]"></div>
                    <div id="itemsContainer" class="lw-list-items-container" role="presentation">
                        <div id="placeholder" class="lw-placeholder" role="presentation" aria-hidden="true">[[placeholder]]</div>
                        <div id="itemsInnerContainer" inner-h-t-m-l=\'[[innerHTML]]\' class="lw-list-items-inner-container" role="presentation">
                            <content></content>
                        </div>
                        <input type="text" id="editInput" class= "lw-hidden"/>
                    </div>
                    <lw-scroll-bar wait right-to-left="[[rightToLeft]]" id="verticalScrollBar" disabled="[[disabled]]" orientation="vertical"></lw-scroll-bar>
                    <lw-scroll-bar wait right-to-left="[[rightToLeft]]" id="horizontalScrollBar" disabled="[[disabled]]"></lw-scroll-bar>
                    <div id="loadingIndicatorContainer" class="lw-loader-container" role="presentation">
    ...
    _mouseLeaveHandler() {
        const that = this,
            itemsWithFeedback = [].slice.call(that.getElementsByClassName('lw-list-item-bottom-line-feedback'));
    ...
    _downHandler(event) {
        ...
        if ((target instanceof LW.ListItem) && !target.disabled && !target.hidden && !(target instanceof LW.ListItemsGroup)) {
        ...
        LW.ListBox.DragDrop.StartTime = new Date();
        LW.ListBox.DragDrop.StartPosition = { left: event.pageX, top: event.pageY };
        ...
        if (that.hasRippleAnimation) {
            if (!item.readonly && !item.disabled) {
                LW.Utilities.Animation.Ripple.animate(target.$.overlay, event.pageX, event.pageY);
            }
        }

        if (!LW.Utilities.Core.isMobile && that.selectionChangeAction === 'press') {
            that._focus(item);
            that.select(item);
        }

        if (!LW.ListBox.DragDrop.Feedback && that.allowDrag) {
            LW.ListBox.DragDrop.DraggedFrom = that;
            LW.ListBox.DragDrop.Feedback = document.createElement('span');

            if (that.rightToLeft) {
                LW.ListBox.DragDrop.Feedback.setAttribute('right-to-left', true);
            }
            else {
                LW.ListBox.DragDrop.Feedback.removeAttribute('right-to-left');
            }

            if (that.dragFeedbackFormatFunction) {
                LW.ListBox.DragDrop.Feedback.innerHTML = that.dragFeedbackFormatFunction(item);
            }
            else {
                LW.ListBox.DragDrop.Feedback.appendChild(item.offsetHeight ? item.cloneNode(true) : that._createItem(item));
            }

            LW.ListBox.DragDrop.Item = item;
            LW.Utilities.Extend(LW.ListBox.DragDrop.Feedback).addClass('lw-popup lw-list-item lw-list-item-feedback');
        }
    }
    ...
    _dblclickEventHandler(event) {
        ...
        const target = event.target.closest('lw-list-item');

        if (!(target instanceof LW.ListItem) || target.disabled || target.readonly) {
            that.$editInput.addClass('lw-hidden');
            return;
        }
        ...
        if (that.$editInput.hasClass('lw-hidden')) {
            that.$editInput.removeClass('lw-hidden');
        }
    ...
    _editInputEventHandler(event) {
        ...
        if (event.type === 'blur' || (event.type === 'keydown' && event.key === 'Escape')) {
            !that.$editInput.hasClass('lw-hidden') ? that.$editInput.addClass('lw-hidden') : undefined;
            that.focus();
            return;
        }

        if (event.type === 'keydown' || that.$editInput.hasClass('lw-hidden')) {
            return;
        }
    ...
    _pointerUpHandler(customEvent) {
        ...
        if (that.shadowRoot || that.isInShadowDOM) {
            target = event.originalEvent ? event.originalEvent.composedPath()[0] : event.composedPath()[0];
        }
        ...
        const select = (item) => {
            ...
            listBox._focus(item);
            listBox.select(item);
            LW.ListBox.DragDrop.StartPosition = null;
        ...
        if ((event.target instanceof LW.ListItem) && !event.target.disabled && !(event.target instanceof LW.ListItemsGroup)) {
            select(target.dataItem);
            return;
        }

        while (target.parentElement) {
            if ((target instanceof LW.ListItem) && !target.disabled && !(target instanceof LW.ListItemsGroup)) {
                select(target.dataItem);
                return;
            }
        ...
        if (this.shadowRoot && this.shadowDomMode === 'shadow') {
            ...
            for (let i = 0; i < composedPath.length; i++) {
                ...
                if (htmlElement instanceof LW.ListItem && !htmlElement.disabled) {
    ...
    _upHandler(event) {
        ...
        if (that.disabled || that.readonly || !LW.ListBox.DragDrop.StartPosition) {
            return;
        }

        if (LW.Utilities.Core.isMobile && !LW.ListBox.DragDrop.Dragging) {
            const left = Math.abs(event.pageX - LW.ListBox.DragDrop.StartPosition.left);
            const top = Math.abs(event.pageY - LW.ListBox.DragDrop.StartPosition.top);
            let canSelect = (left >= 0 && left <= 3) && (top >= 0 && top <= 3);

            if (LW.Utilities.Core.isMobile) {
                if (new Date() - LW.ListBox.DragDrop.StartTime >= 300) {
                    canSelect = false;
                    LW.ListBox.DragDrop.StartTime = null;
                }
            }

            if (canSelect) {
                const closestListBox = event.originalEvent.target.closest('lw-list-box');
                ...
                let target = event.originalEvent.target;
                if ((event.target instanceof LW.ListItem) && !target.readonly && !event.target.disabled && !(event.target instanceof LW.ListItemsGroup)) {
                    event.stopPropagation();
                }
                else {
                    while (target.parentElement) {
                        if ((target instanceof LW.ListItem) && !target.readonly && !target.disabled && !(target instanceof LW.ListItemsGroup)) {
                            event.stopPropagation();
                            break;
                        }
        ...
        else if (!LW.Utilities.Core.isMobile && !LW.ListBox.DragDrop.Dragging && that.selectionChangeAction === 'release') {
            const closestListBox = (that.shadowRoot || that.isInShadowDOM ? event.originalEvent.composedPath()[0] : event.originalEvent.target).closest('lw-list-box');

            closestListBox ? closestListBox._pointerUpHandler(event) : that._pointerUpHandler(event);
        }

        if (LW.ListBox.DragDrop.Feedback) {
            if (LW.ListBox.DragDrop.Feedback.parentNode) {
                LW.ListBox.DragDrop.Feedback.parentNode.removeChild(LW.ListBox.DragDrop.Feedback);
            }
            LW.ListBox.DragDrop.Feedback = null;
            let dropTarget;

            if (LW.Utilities.Core.isMobile && LW.ListBox.DragDrop.Dragging) {
                for (let j = 0; j < LW.ListBox.DragDrop.ListBoxes.length; j++) {
                    const listBox = LW.ListBox.DragDrop.ListBoxes[j];
                    const rect = listBox.getBoundingClientRect();

                    if (!listBox.allowDrop || listBox.closest('.lw-visibility-hidden')) {
                        continue;
                    }
                        ...
                        for (let i = 0; i < visibleItems.length; i++) {
                            ...
                            if (visibleItems.indexOf(item) === visibleItems.length - 1 || item.parentNode.lastElementChild === item) {
                                if (event.clientY >= rect.top + item.offsetTop - listBox.scrollTop + (itemRect.height / 2) && event.clientY <= rect.top - listBox.scrollTop + item.offsetTop + itemRect.height) {
                                    LW.ListBox.DragDrop.DropDetails = { item: item, position: 'bottom' };
                                    break;
                                }
                            }
                            if (event.clientY >= rect.top + item.offsetTop - listBox.scrollTop && event.clientY <= rect.top + item.offsetTop + itemRect.height - listBox.scrollTop) {
                                LW.ListBox.DragDrop.DropDetails = { item: item, position: 'top' };
                                break;
                            }
                        }
                    }
            ...
            if (LW.ListBox.DragDrop.Dragging) {
                if (LW.ListBox.DragDrop.DropDetails) {
                    let dropItem = LW.ListBox.DragDrop.DropDetails.item,
                        dragItem = LW.ListBox.DragDrop.Item;
                    ...
                    dropTarget = !dropTarget && target.closest ? target.closest('lw-list-box') : dropTarget;

                    document.body.classList.remove('lw-dragging');
                    dropItem.$.removeClass('lw-list-item-line-feedback');
                    dropItem.$.removeClass('lw-list-item-bottom-line-feedback');
                    ...
                    if (LW.ListBox.DragDrop.Item !== dropItem && (dropTarget === dropItem.ownerListBox || LW.Utilities.Core.isMobile)) {
                        if (dragItem.ownerListBox.dropAction !== 'none') {
                            if (dropItem.ownerListBox !== dragItem.ownerListBox && dragItem.ownerListBox.dropAction === 'move') {
                                LW.ListBox.DragDrop.Item.ownerListBox.removeChild(LW.ListBox.DragDrop.Item);
                            }
                            ...
                            LW.ListBox.DragDrop.Item.group = dropItem.group;
                            ...
                            if (LW.ListBox.DragDrop.DropDetails.position === 'top') {
                    ...
                    const item = LW.ListBox.DragDrop.Item;
                    const listBox = item.ownerListBox;
            ...
            delete LW.ListBox.DragDrop.DraggedFrom;
            LW.ListBox.DragDrop.Dragging = false;
            LW.ListBox.DragDrop.Item = null;
            LW.ListBox.DragDrop.DropDetails = null;
        }

        for (let i = 0; i < LW.ListBox.DragDrop.ListBoxes.length; i++) {
            const listBox = LW.ListBox.DragDrop.ListBoxes[i];
    ...
    _touchmoveHandler(event) {
        if (LW.ListBox.DragDrop.Dragging && event.cancelable) {
            event.preventDefault();
            event.stopPropagation();
            return;
        }
    ...
    _applyItemFeedback(event) {
        const that = this,
            ...
            targetItem = target.closest ? target.closest('lw-list-item') : target;

        //Event.target doesn't work on Mobile. This is used just to the Drop logic inside upHandler
        if (LW.Utilities.Core.isMobile) {
            LW.ListBox.DragDrop.DropDetails = { item: LW.ListBox.DragDrop.Item, position: 'bottom' };
            return;
        }
    ...
    _moveHandler(event) {
        ...
        if (!LW.ListBox.DragDrop.Feedback || LW.ListBox.DragDrop.DraggedFrom !== that) {
            return;
        }
        ...
        if (!LW.ListBox.DragDrop.Dragging) {
            const left = Math.abs(event.pageX - LW.ListBox.DragDrop.StartPosition.left);
            const top = Math.abs(event.pageY - LW.ListBox.DragDrop.StartPosition.top);
            const timeOut = new Date() - LW.ListBox.DragDrop.StartTime;
            ...
            if (canStartDrag) {
                LW.ListBox.DragDrop.Dragging = true;
                LW.ListBox.DragDrop.Feedback.style.left = dragOffset[0] + event.pageX + 'px';
                LW.ListBox.DragDrop.Feedback.style.top = dragOffset[1] + event.pageY + 'px';


                that.getShadowRootOrBody().appendChild(LW.ListBox.DragDrop.Feedback);
                event.preventDefault();
                event.stopPropagation();
   ...
   */
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            case 'innerHTML':
                that.clearSelection();
                that._dataBind();
                that.innerHTML = that.content = LW.Utilities.Core.html(that.$.content);
                break;
            case 'dataSource':
            case 'valueMember':
            case 'groupMember':
            ...
            case 'itemTemplate': {
                ...
                const items = that.querySelectorAll('lw-list-item');
    ...
    */
    ready() {
        ...
        that._areCSSVarsSupported = LW.Utilities.Core.CSSVariablesSupport();
        that.isUpdating = 0;

        if (that._scrollView) {
            that._scrollView = new LW.Utilities.Scroll(that, that.$.horizontalScrollBar, that.$.verticalScrollBar);
            that._scrollView.hScrollBar.$.addClass('lw-hidden');
            that._scrollView.vScrollBar.$.addClass('lw-hidden');
        }
    ...
    attached() {
        ...
        if (!LW.ListBox.DragDrop) {
            LW.ListBox.DragDrop = { ListBoxes: [] };
        }

        LW.ListBox.DragDrop.ListBoxes.push(that);
    ...
    detached() {
        ...
        if (LW.ListBox.DragDrop && LW.ListBox.DragDrop.ListBoxes) {
            const index = LW.ListBox.DragDrop.ListBoxes.indexOf(that);
            LW.ListBox.DragDrop.ListBoxes.splice(index, 1);
        }
    }
    ...
    _createItem(item, index, recycling) {
        ...
        const displayMode = that.selectionMode === 'checkBox' || that.selectionMode === 'radioButton' ? that.selectionMode : 'plain';
        const htmlItem = recycling ? {} : document.createElement('lw-list-item');
    ...
    get _scrollHeight() {
        ...
        if (that._items.length === 0 || that._items.filter(i => !i.hidden).length < 2) {
            return 0;
        }
        ...
        //if (scrollHeight) {
        //    bottomOffset = 2 * (parseInt(getComputedStyle(that.$.itemsInnerContainer).getPropertyValue('--lw-list-item-vertical-offset')) || 0);
        //}
    ...
    _dataBind() {
        ...
        const items = that.shadowRoot ? that.shadowRoot.querySelectorAll('lw-list-item') : that.querySelectorAll('lw-list-item');
        const itemsReady = function () {
            that.isUpdating++;
    ...
    _initializeInnerHTML() {
        ...
        if (that.$.itemsInnerContainer.innerHTML.indexOf('<option') >= 0) {
            that.$.itemsInnerContainer.innerHTML = that.$.itemsInnerContainer.innerHTML.replace(/<optgroup/ig, '<lw-list-items-group').replace(/<\/optgroup/ig, '<\/lw-list-items-group').replace(/<option/ig, '<lw-list-item').replace(/<\/option/ig, '<\/lw-list-item');
        }
        else if (that.$.itemsInnerContainer.innerHTML.indexOf('<ul') >= 0) {
            that.$.itemsInnerContainer.innerHTML = that.$.itemsInnerContainer.innerHTML.replace(/<ul/ig, '<lw-list-items-group').replace(/<\/ul/ig, '<\/lw-list-items-group').replace(/<li/ig, '<lw-list-item').replace(/<\/li/ig, '<\/lw-list-item');
        }
    }
    ...
    _initializeHtmlListItems() {
        ...
        const htmlItems = that.shadowRoot && that.shadowDomMode === 'shadow' ? that.shadowRoot.querySelectorAll('lw-list-item') : that.querySelectorAll('lw-list-item');
        const displayMode = that.selectionMode === 'checkBox' || that.selectionMode === 'radioButton' ? that.selectionMode : 'plain';
    ...
    _group() {
        ...
        if (!that.isVirtualized) {
            ...
            for (let i = 0; i < groups.length; i++) {
                ...
                const groupElement = document.createElement('lw-list-items-group');
    ...
    _initializeHtmlListGroups() {
        const that = this;
        const htmlListItemsGroups = that.$.itemsInnerContainer.querySelectorAll('lw-list-items-group');
    ...
    select(item) {
        ...
        switch (that.selectionMode) {
            ...
            case 'radioButton':
                ...
                if (that.$.itemsInnerContainer.querySelectorAll('lw-list-items-group').length > 0) {
    ...
    _filterItems(noSelectionRefresh, callback, ignoreFilterQueryLength) {
        ...
        const updateListBox = function () {
            ...
            if (that.grouped && !that.isVirtualized) {
                for (let i = 0; i < that._groups.length; i++) {
                    const group = that._groups[i];
                    const items = [].concat(...group.querySelectorAll('lw-list-item'));

                    let canHide = true;

                    for (let j = 0; j < items.length; j++) {
                        if (!items[j].$.hasClass('lw-hidden')) {
                            canHide = false;
                        }
                    }

                    if (canHide) {
                        group.$.addClass('lw-hidden');
                    }
                    else {
                        group.$.removeClass('lw-hidden');
                    }
                }
            }
    ...
    */
    ...
    _getScrollLeft(scrollLeft, scrollWidth) {
        ...
        if (LW.Utilities.Core.Browser.Chrome) {
            if (!scrollWidth) {
                const horizontalOffset = 2 * (parseInt(getComputedStyle(that.$.itemsInnerContainer).getPropertyValue('--lw-list-item-horizontal-offset')) || 0);
    ...
    */
    clearItems() {
        ...
        that.$placeholder.removeClass('lw-hidden');
    ...
    */
    _setLoadingIndicatorVisibility() {
        ...
        if (that.displayLoadingIndicator) {
            that.$loadingIndicatorContainer.hasClass('lw-visibility-hidden') ?
                that.$loadingIndicatorContainer.removeClass('lw-visibility-hidden') : that.$loadingIndicatorContainer.removeClass('lw-hidden');
            return;
        }

        //Check for old browsers.
        that._areCSSVarsSupported ?
            that.$loadingIndicatorContainer.addClass('lw-hidden') : that.$loadingIndicatorContainer.addClass('lw-visibility-hidden');
    }
    ...
    */
    _refreshLayout() {
        const that = this;
        const source = that._filteredItems || that._items;
        ...
        that.$placeholder.addClass('lw-hidden');

        const horizontalOffset = 2 * (parseInt(getComputedStyle(that.$.itemsContainer).getPropertyValue('--lw-list-item-horizontal-offset')) || 0);
