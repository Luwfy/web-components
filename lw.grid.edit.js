...
LW.Utilities.Assign( 'Grid.Edit', class Edit {
    _renderCommandBar() {
        const that = this;
        ...
        const renderCommandBar = ( commandBar ) => {
            for ( let item in dataSource ) {
                ...
                commandBarItem.classList.add( 'lw-grid-command-item' );

                commandBarItem.label = that.localize( item );

                if ( properties.icon ) {
                    commandBarItem.innerHTML = '<span class="lw-grid-icon ' + properties.icon + '"></span>' + '<span class="lw-grid-label">' + commandBarItem.label + '</span>';
                }

                if ( !properties.visible ) {
                    commandBarItem.classList.add( 'lw-hidden' );
                }
    ...
    _renderAddNewRow() {
        ...
        const newNearRow = new LW.Grid.Row( {data: {}, index: -1, grid: that, freeze: 'near', visible: that.editing.addNewRow.position !== 'far', autoGenerated: true, addNewRow: true} );
        const newFarRow = new LW.Grid.Row( {data: {}, index: -2, grid: that, freeze: 'far', visible: that.editing.addNewRow.position !== 'near', autoGenerated: true, addNewRow: true} );
    ...
    _insertNewRowBefore( callback, insertAtBottom ) {
        const that = this;
        const row = new LW.Grid.Row( {index: insertAtBottom ? 0 : -1, id: LW.Utilities.Core.createGUID(), grid: that} );
    ...
    _getCommandColumnCommandsTemplate() {
        ...
        for ( let item in dataSource ) {
            ...
            if ( !isCommandSet ) {
                commandColumnItem += ' item="' + item + '" command="' + properties.command + '"' + ' class="lw-grid-command-item">';
                isCommandSet = true;
            }
            ...
            if ( showIcon && showLabel ) {
                commandColumnItem += '<span class="lw-grid-icon ' + properties.icon + '"></span>';
    ...
    _updateCommandColumnCommandsVisibility( element, row ) {
        ...
        for ( let i = 0; i < commands.length; i++ ) {
            ...
            if ( visible === true ) {
                command.classList.remove( 'lw-hidden' );
            }
            else if ( visible === false ) {
                command.classList.add( 'lw-hidden' );
            }
                ...
                if ( !that.editing.dialog.enabled && ( that.editing.editRow === row || ( that.editing.editCell && that.editing.editCell.row === row ) ) ) {
                    if ( item === 'commandColumnEdit' ) {
                        ...
                        commands[ i ].classList.remove( 'lw-hidden' );
                    }
                }
                else {
                    if ( item === 'commandColumnEdit' ) {
                        ...
                        commands[ i ].classList.add( 'lw-hidden' );
                    }
                }
            }
        }
    ...
    _getCustomCellEditor( cell ) {
        ...
        if ( !that._cellEditors[ id ] ) {
            ...
            element.classList.add( 'lw-grid-cell-editor' );
    ...
    _getTextAreaCellEditor( cell ) {
        ...
        textarea.classList.add( 'lw-input' );
        element.classList.add( 'lw-grid-cell-editor' );
        element.classList.add( 'lw-grid-text-area-cell-editor' );
        expander.classList.add( 'nav' );

        expander.classList.add( 'lw-icon-resize-full', 'lw-grid-icon' );
        ...
        expander.onclick = function () {
            ...
            const scrollBar = document.createElement( 'lw-scroll-bar' );
            ...
            textAreaContainer.classList.add( 'lw-grid-text-area-container' );
            textAreaContainer.appendChild( scrollBar );

            popupTextArea.value = textarea.value;
            popupTextArea.classList.add( 'lw-input' );

            expander.textAreaEditorDialog = textAreaEditorDialog;
            expander.popupTextArea = popupTextArea;

            closeButton.classList.add( 'close-button' );
            closeButton.classList.add( 'lw-grid-icon' );
            closeButton.classList.add( 'lw-icon-cancel-circled' );
            ...
            const updateScrollBar = function () {
                ...
                if ( popupTextArea.offsetHeight >= popupTextArea.scrollHeight ) {
                    scrollBar.classList.add( 'lw-hidden' );
                }
                else {
                    scrollBar.classList.remove( 'lw-hidden' );
                }
            ...
            textAreaEditorDialog.classList.add( 'lw-grid-text-area-dialog' );
    ...
    _getInputCellEditor( cell ) {
        ...
        if ( !that._cellEditors[ id ] ) {
            ...
            element.classList.add( 'lw-input' );
            element.classList.add( 'lw-grid-cell-editor' );
            element.classList.add( 'lw-grid-input-cell-editor' );
    ...
    _getAutoCompleteCellEditor( cell ) {
        ...
        if ( !that._cellEditors[ id ] ) {
            const element = document.createElement( 'lw-input' );
            ...
            element.classList.add( 'lw-grid-cell-editor' );
            element.classList.add( 'lw-grid-auto-complete-cell-editor' );
    ...
    _getDateTimePickerCellEditor( cell ) {
        ...
        if ( !that._cellEditors[ id ] ) {
            const element = document.createElement( 'lw-date-time-picker' );
            ...
            element.classList.add( 'lw-grid-cell-editor' );
            element.classList.add( 'lw-grid-date-time-picker-cell-editor' );
    ...
    _getNumberInputCellEditor( cell ) {
        ...
        if ( !that._cellEditors[ id ] ) {
            ...
            numberInput.classList.add( 'lw-input' );
            element.classList.add( 'lw-grid-cell-editor' );
            element.classList.add( 'lw-grid-number-input-cell-editor' );
    ...
    _getCheckBoxCellEditor( cell ) {
        ...
        if ( !that._cellEditors[ id ] ) {
            ...
            element.classList.add( 'lw-input' );
            element.classList.add( 'lw-grid-cell-editor' );
            element.classList.add( 'lw-grid-check-box-cell-editor' );
            element.tabIndex = 0;
        ...
        const parseValue = function ( value ) {
            switch ( cell.column.dataType ) {
                ...
                case 'boolean':
                    ...
                    break;
                case 'date':
                    value = new LW.Utilities.DateTime( value );
    ...
    _createOverlay() {
        ...
        overlay.classList.add( 'lw-grid-dialog-overlay' );
    ...
    _createDialog( options ) {
        ...
        dialog.classList.add( 'lw-window', 'lw-grid-dialog' );
        ...
        dialog.innerHTML =
            `<div class="lw-container">
                   <div class ="lw-content-container" lw-id="container">
                        <div id="headerSection" class="lw-header-section">
                            <div class="lw-header"></div>
                            <div class="lw-buttons-container">
                                <button class ="lw-button lw-element lw-close-button" aria-label="Close" lw-id="closeButton"></button>
                            </div>
                        </div>
                        <div class="lw-content"><div></div></div>
                        <div class ="lw-footer">
                            <div class ="lw-stack-layout right spacing">
                                <lw-button class ="lw-confirm-button item primary">Ok</lw-button>
                                <lw-button class ="lw-cancel-button item">Cancel</lw-button>
                            </div>
                        </div>
                    </div>
              </div>
        ...
        dialog.content = dialog.querySelector( '.lw-content' ).firstChild;
        dialog.footer = dialog.querySelector( '.lw-footer' );
        dialog.header = dialog.querySelector( '.lw-header' );
        dialog.btnConfirm = dialog.querySelector( '.lw-confirm-button' );
        dialog.btnCancel = dialog.querySelector( '.lw-cancel-button' );
        dialog.btnClose = dialog.querySelector( '.lw-close-button' );
        const overlay = document.createElement( 'div' );
    ...
    _openAddRowDialog( row, command ) {
        ...
        if ( !that._dialogAddRow ) {
            ...
            dialog.btnConfirm.onclick = function () {
                ...
                const row = new LW.Grid.Row( {index: that.rows.length + index, grid: that, data: data} );
    ...
    _beginCellEdit( cell, event ) {
        ...
        if ( that._selection.selectionRect ) {
            that._selection.selectionRect.classList.add( 'lw-visibility-hidden' );
        }
        ...
        switch ( cell.editor.template ) {
            ...
            case 'checkBox': {
                ...
                if ( cell.template === 'checkBox' && event ) {
                    ...
                    if ( elements[ 0 ].classList.contains( 'lw-input' ) ) {
    ...
    _onCellClick( cell, event ) {
        ...
        if ( that.editing.commandColumn.visible ) {
            if ( cell.column.commandColumn ) {
                ...
                if ( !commandItem ) {
                    commandItem = cell.element.querySelector( '.lw-grid-command-item' );
                }

                if ( commandItem && !commandItem.classList.contains( 'lw-hidden' ) ) {
    ...
    */
    ...
    addUnboundRow( count, position ) {
        ...
        for ( let i = 0; i < count; i++ ) {
            const id = LW.Utilities.Core.createGUID().replace( /-/ig, '' );
            const row = new LW.Grid.Row( {index: -1, unbound: true, id: id, grid: that} );
    ...
    endEdit() {
        ...
        if ( that._selection.selectionRect ) {
            that._selection.selectionRect.classList.remove( 'lw-visibility-hidden' );
        }
