...
LW.Utilities.Assign('DataExporter', class DataExporter {
...
<body>
    ...
    exportToImage(data, fileName, fileExtension, callback) {
        ...
        try {
            ...
            throw new Error('lw-grid: Missing reference to \'html2canvas.min.js\'.');
        }
        ...
        function checkIframePopulated() {
            if (!iframe.contentDocument.body || !iframe.contentDocument.body.firstElementChild) {
                ...
                iframe.contentWindow.html2canvas(iframe.contentDocument.body.firstElementChild).then(canvas => {
                    const draw = new LW.Utilities.Draw(document.createElement('div'));
    ...
    formatDate(value, format) {
        ...
        try {
            return new LW.Utilities.DateTime(value).toString(format);
        }
        catch (error) {
            return value;
        }
    ...
    getColumnStyle() {
        ...
        for (let prop in headerDefinition) {
            ...
            if (sampleRecord[prop]) {
                ...
                for (let columnProp in value) {
                    if (value.hasOwnProperty(columnProp)) {
                        const css = LW.Utilities.Core.toDash(columnProp) + ': ' + value[columnProp] + '; ';
            ...
            if (prop === 'height' && that.complexHeader) {
                ...
                styles.header += LW.Utilities.Core.toDash(prop) + ': ' + headerDefinition[prop] + '; ';
            }
        }
        ...
        for (let prop in columnsDefinition) {
            ...
            if (sampleRecord[prop]) {
                ...
                for (let columnProp in value) {
                    if (isNaN(columnProp) && value.hasOwnProperty(columnProp) && columnProp !== 'format') {
                        styles['column' + prop] += LW.Utilities.Core.toDash(columnProp) + ': ' + value[columnProp] + '; ';
                    }
                }
            ...
            styles.column += LW.Utilities.Core.toDash(prop) + ': ' + value + '; ';
        }
    ...
    getRowStyle() {
        ...
        for (let prop in rowsDefinition) {
            ...
            if (!isNaN(prop)) {
                ...
                for (let rowProp in value) {
                    if (value.hasOwnProperty(rowProp)) {
                        styles['row' + prop] += LW.Utilities.Core.toDash(rowProp) + ': ' + value[rowProp] + '; ';
                    }
                }
            ...
            styles.row += LW.Utilities.Core.toDash(prop) + ': ' + rowsDefinition[prop] + '; ';
        }
    ...
    getTableStyle() {
        ...
        for (let prop in style) {
            if (style.hasOwnProperty(prop) && ['header', 'columns', 'rows'].indexOf(prop) === -1) {
                generatedStyle += LW.Utilities.Core.toDash(prop) + ': ' + style[prop] + '; ';
            }
        }
