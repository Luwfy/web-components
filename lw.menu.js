 ...
 */
LW('lw-menu-item', class MenuItem extends LW.BaseElement {
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        if (that.tagName.toLowerCase() === 'lw-tree-item') {
            return;
        }
 ...
 */
LW('lw-menu-items-group', class MenuItemsGroup extends LW.BaseElement {
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        if (that.tagName.toLowerCase() === 'lw-tree-items-group') {
            return;
        }
        ...
        switch (propertyName) {
            ...
            case 'checkMode':
                ...
                break;
            case 'disabled': {
                if (LW.ListMenu && parentMenu instanceof LW.ListMenu) {
                    if (parentMenu._view) {
 ...
 * Menu custom element.
 */
LW('lw-menu', class Menu extends LW.BaseElement {
    // Menu's properties.
    static get properties() {
        return {
            'autoCloseDelay': {
    ...
    template() {
        return `<div id="container" role="presentation">
                    ...
                    <lw-repeat-button id="scrollButtonNear" class="lw-menu-scroll-button lw-spin-button lw-scroll-button-near lw-hidden" animation="[[animation]]" unfocusable right-to-left="[[rightToLeft]]">
                        <div id="arrowNear" class="lw-arrow" aria-hidden="true"></div>
                    </lw-repeat-button>
                    <div id="mainContainer" class="lw-menu-main-container" role="presentation">
                        <content></content>
                    </div>
                    <lw-repeat-button id="scrollButtonFar" class="lw-menu-scroll-button lw-spin-button lw-scroll-button-far lw-hidden" animation="[[animation]]" unfocusable right-to-left="[[rightToLeft]]">
    ...
    attached() {
        ...
        if (!that.isCompleted || !that.isRendered ||
            that._element === 'tree' || LW.ListMenu && that instanceof LW.ListMenu) {
            return;
        }
    ...
    detached() {
        ...
        if (that._element === 'tree' || LW.ListMenu && that instanceof LW.ListMenu) {
            return;
        }
     ...
     * Adds an item.
     *
     * @param {HTMLElement} item A lw-menu-item to add to the Menu.
    ...
    addItem(item, parent) {
        ...
        if (parent === undefined) {
            ...
            if (parent instanceof LW.MenuItemsGroup === false) {
                return;
            }
        ...
        if (item instanceof LW.MenuItemsGroup && (that.mode === 'tree' || that._minimized)) {
            const arrows = item.getElementsByClassName('lw-menu-items-group-arrow');

            for (let i = 0; i < arrows.length; i++) {
                arrows[i].className = 'lw-menu-items-group-arrow down lw-arrow-down';
            }
     ...
     * Checks an item.
     *
     * @param {HTMLElement/String} item lw-menu-item/lw-menu-items-group (or its id or numeric path).
     */
    checkItem(item) {
        const that = this;
    ...
    close() {
        ...
        if (that.mode !== 'dropDown' || that.opened === false && that.$.hasClass('lw-visibility-hidden')) {
            return;
        }
     ...
     * Collapses an item.
     *
     * @param {HTMLElement/String} item Optional lw-menu-item/lw-menu-items-group (or its id or numeric path).
    ...
    collapseItem(item, animation) {
        ...
        if (item !== undefined) {
            ...
            if (item === undefined || item instanceof LW.MenuItem) {
                return;
            }
        }
        else {
    ...
    expandItem(item, animation) {
        ...
        if (item === undefined ||
            (item instanceof LW.MenuItemsGroup &&
        ...
        for (let i = 0; i < stack.length; i++) {
            ...
            if (i === stack.length - 1 && currentStackItem instanceof LW.MenuItem) {
    ...
    maximize() {
        ...
        if (that.enableShadowDOM && !that.$.mainContainer.id) {
            that.$.mainContainer.id = that.$.mainContainer.getAttribute('lw-id');
        }
        ...
        if (that.mode !== 'tree') {
            that.$mainContainer.addClass('lw-menu-main-container');
            that.$mainContainer.removeClass('lw-menu-minimized-items-container');

            const arrows = that.$.mainContainer.getElementsByClassName('lw-menu-items-group-arrow');
        ...
        that.$mainContainer.removeClass('lw-visibility-hidden');
    ...
    minimize() {
        ...
        that.$mainContainer.removeClass('lw-menu-main-container');
        that.$mainContainer.addClass('lw-visibility-hidden');
        ...
        setTimeout(function () {
            that.$mainContainer.addClass('lw-menu-minimized-items-container');
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            ...
            case 'mode': {
                ...
                if (that.dropDownAppendTo !== null) {
                    for (let i = 0; i < that._containers.length; i++) {
                        ...
                        container.setAttribute(LW.Utilities.Core.toDash(propertyName), newValue);
            ...
            case 'theme':
                if (that.dropDownAppendTo === null || LW.ListMenu && that instanceof LW.ListMenu) {
                    return;
                }
    ...
    _appendMinimizedContainerToExternalElement(itemsContainer) {
        ...
        itemsContainer.$.addClass('lw-menu-drop-down lw-drop-down');
        itemsContainer.$.addClass('lw-drop-down-repositioned');
    ...
    _appendMinimizedContainerToMenu(itemsContainer, sibling) {
        ...
        itemsContainer.$.removeClass('lw-menu-drop-down lw-drop-down');
        itemsContainer.$.removeClass('lw-drop-down-repositioned');
    ...
    _applyGrouping(item, noRecursion) {
        ...
        for (let i = 0; i < itemChildren.length; i++) {
            ...
            if (currentItem instanceof LW.MenuItemsGroup && noRecursion === undefined) {
    ...
    _applyMinimizeIconTemplate(newValue, oldValue) {
        ...
        if (newValue === null) {
            ...
            that.$customIconContainer.addClass('lw-hidden');
            that.$.customIconContainer.innerHTML = '';

            that.$hamburgerIconLineTop.removeClass('lw-hidden');
            that.$hamburgerIconLineCenter.removeClass('lw-hidden');
            that.$hamburgerIconLineBottom.removeClass('lw-hidden');
        }
        else {
            ...
            if (potentialHTMLTemplate !== null && potentialHTMLTemplate.tagName.toLowerCase() === 'template') {
                ...
                that.$hamburgerIconLineTop.addClass('lw-hidden');
                that.$hamburgerIconLineCenter.addClass('lw-hidden');
                that.$hamburgerIconLineBottom.addClass('lw-hidden');

                that.$.customIconContainer.innerHTML = '';
                that.$.customIconContainer.appendChild(templateContent);
                that.$customIconContainer.removeClass('lw-hidden');
            }
            else {
    ...
    _applyTreeMode() {
        const that = this,
            arrows = that.$.mainContainer.getElementsByClassName('lw-menu-items-group-arrow');
        ...
        if (that.isCompleted && that.isRendered) {
            for (let i = 0; i < arrows.length; i++) {
                arrows[i].className = 'lw-menu-items-group-arrow down';
            }
        }
    ...
    _arrowRightHandler(level, mode, focusedItem, lastOpenedContainer) {
        ...
        if (level === 1) {
            ...
            if (focusedItem instanceof LW.MenuItemsGroup) {
    ...
    _checkOverflow(element, horizontal, scrollButtons) {
        ...
        if (overflow === 'auto') {
            ...
            scrollButtons[0].$.addClass('lw-hidden');
            scrollButtons[1].$.addClass('lw-hidden');
        }
        ...
        if (overflowing) {
            if (overflow === 'auto') {
                ...
                if (showNear) {
                    scrollButtons[0].$.removeClass('lw-hidden');
                }

                if (showFar) {
                    scrollButtons[1].$.removeClass('lw-hidden');
                }
    ...
    _closeSubContainersDefaultMode(level, container, preserveClass) {
        ...
        function applyClasses(i) {
            ...
            menuItemsGroup.$.removeClass('lw-menu-items-group-opened');
            menuItemsGroup.$.removeClass('lw-menu-items-group-expanded');
            menuItemsGroup.setAttribute('aria-expanded', false);
            openedContainers[i].$.addClass('lw-visibility-hidden');
    ...
    _closeSubContainersTreeMode(level, container, preserveClass, fireEvent, modeChanged) {
        ...
        menuItemsGroup.$.removeClass('lw-' + that._element + '-items-group-opened');

        if (modeChanged || !that.hasAnimation) {
            menuItemsGroup.$.removeClass('lw-' + that._element + '-items-group-expanded');
            menuItemsGroup.setAttribute('aria-expanded', false);
            container.$.addClass('lw-visibility-hidden');
    ...
    _collapseSection(element) {
        ...
        requestAnimationFrame(function () {
            ...
            requestAnimationFrame(function () {
                ...
                if (that.tagName.toLowerCase() === 'lw-tree') {
                    element.$.addClass('lw-visibility-hidden');
                }
    ...
    _createElement() {
        ...
        const items = (that.shadowRoot || that).querySelectorAll('lw-menu-item, lw-menu-items-group');
    ...
    /**
     * Creates the necessary HTML structure for a lw-menu-item/lw-menu-items-group.
     */
    _createItemHTMLStructure(currentItem, level, item, index) {
        ...
        labelContainer.className = 'lw-' + that._element + '-item-label-container';
        labelElement.className = 'lw-' + that._element + '-item-label-element';
        ...
        for (let i = 0; i < currentItemChildNodes.length; i++) {
            ...
            if (currentNode instanceof LW.MenuItem || currentNode instanceof LW.MenuItemsGroup) {
                break;
            }
        ...
        if (currentItem instanceof LW.MenuItemsGroup && currentItem.titleLabel === undefined) {
        ...
        if (currentItem.shortcut && currentItem instanceof LW.MenuItem) {
            const shortcutElement = document.createElement('div');

            shortcutElement.id = currentItem.id + 'Shortcut';
            shortcutElement.className = 'lw-' + that._element + '-item-shortcut';
    ...
    /**
     * Creates the dropdown container of lw-menu-items-group.
     */
    _createMenuItemsGroupContainer(item, level) {
        ...
        container.className = 'lw-' + that._element + '-drop-down lw-visibility-hidden';
        ...
        itemContainer.className = 'lw-' + that._element + '-item-container';
        ...
        if (item.dropDownHeight !== null) {
            const repeatButtonNear = document.createElement('lw-repeat-button'),
                repeatButtonFar = document.createElement('lw-repeat-button');

            repeatButtonNear.className = 'lw-menu-scroll-button lw-spin-button lw-scroll-button-near lw-hidden';
            repeatButtonNear.setAttribute('aria-label', 'Scroll up');
            repeatButtonNear.innerHTML = '<div class="lw-arrow lw-arrow-up" aria-hidden="true"></div>';
            repeatButtonNear.animation = that.animation;
            repeatButtonNear.unfocusable = true;
            repeatButtonNear.rightToLeft = that.rightToLeft;
            repeatButtonFar.className = 'lw-menu-scroll-button lw-spin-button lw-scroll-button-far lw-hidden';
            repeatButtonFar.setAttribute('aria-label', 'Scroll down');
            repeatButtonFar.innerHTML = '<div class="lw-arrow lw-arrow-down" aria-hidden="true"></div>';
    ...
    _discardKeyboardHover(overrideCheck) {
        ...
        if (!overrideCheck &&
            that._focusedViaKeyboard instanceof LW.MenuItemsGroup &&
    ...
    _documentUpHandler(event) {
        ...
        if (!closed && that !== activeElement && activeElement.closest('[template-applied]') === null) {
            if (isInsideElement || target.closest('.lw-list-menu-view') === that.$.view) {
                that.focus();
                return;
            }

            const closestDropDown = target.closest('.lw-drop-down-repositioned');
    ...
    _closeOnDocumentInteraction(event) {
        ...
        if (!isInsideElement && target.closest('.lw-menu-drop-down') === null ||
    ...
    _ensureVisible(scrollTarget) {
        ...
        function scrollTo(scrollDimension, dimension) {
            ...
            if (scrollDimension === 'Top' && parent !== mainContainer && !scrollButtons[0].$.hasClass('lw-hidden')) {
    ...
    /**
     * Makes sure a lw-menu-item/lw-menu-items-group is visible by scrolling to it (when "mode" is 'tree').
     */
    _ensureVisibleTreeMode(item, scrollTargetBoundingRect, parent, parentBoundingRect, padding) {
        ...
        const oldHeight = parent.offsetHeight,
            scrollButtonNearHidden = that.$scrollButtonNear.hasClass('lw-hidden');
        let newScrollTop;
        ...
        if (parentBoundingRect.top > scrollTargetBoundingRect.top) {
            ...
            if (that._element === 'tree') {
                itemHeight += parseFloat(getComputedStyle(that).getPropertyValue('--lw-tree-item-vertical-offset')) || 0;
            }
        ...
        if (that._element === 'tree' && item.path === '0') {
            newScrollTop -= parseFloat(getComputedStyle(that).getPropertyValue('--lw-tree-item-vertical-offset'));
        }
        ...
        if (that.overflow === 'auto' && scrollButtonNearHidden && !that.$scrollButtonNear.hasClass('lw-hidden')) {
    ...
    _hamburgerIconClickHandler(event, itemContainer) {
        ...
        if (itemContainer === undefined) {
            if (LW.ListMenu && that instanceof LW.ListMenu) {
        ...
        if (!that._minimizedDropDownOpened) {
            ...
            itemContainer.$.removeClass('lw-visibility-hidden');
    ...
    _handleOverflowChange() {
        ...
        if ((that._minimized || that.dropDownAppendTo === null && !mainContainer.classList.contains('simple') && that.mode !== 'tree') && !(LW.ListMenu && that instanceof LW.ListMenu)) {
            return;
        }
    ...
    _keydownHandlerTreeMode(key) {
        const that = this,
            menuItems = Array.from(that.$.mainContainer.querySelectorAll('lw-menu-item, lw-menu-items-group')),
        ...
        switch (key) {
            ...
            case 'ArrowLeft':
                ...
                if (focusedItem instanceof LW.MenuItem) {
                ...
                break;
            case 'ArrowRight':
                if (!focusedItem || focusedItem instanceof LW.MenuItem) {
                    return;
                }
    ...
    _levelOneNavigateFromLowerLevel(method, focusedItem) {
        ...
        if (firstLevelItem) {
            ...
            if (firstLevelItem instanceof LW.MenuItemsGroup) {
    ...
    _levelOneOpenDropDown(focusedItem) {
        if (focusedItem && focusedItem instanceof LW.MenuItemsGroup) {
    ...
    /**
     * lw-menu-item selection handler.
     */
    ...
    _menuItemsGroupSelectionHandler(closestItemsGroup, event, fireEvent) {
        ...
        if (event.type === 'click' &&
            ((!event.target.classList.contains('lw-' + that._element + '-items-group-arrow') && that._toggleItem(closestItemsGroup)) ||
        ...
        if (that._isContainerOpened(level, container)) {
            ...
            container.$.removeClass('lw-visibility-hidden');
            ...
            closestItemsGroup.$.addClass('lw-' + that._element + '-items-group-opened');
            closestItemsGroup.$.addClass('lw-' + that._element + '-items-group-expanded');
        ...
        if (that.selectionMode === 'mouseenter' && that.mode !== 'tree' && !that._minimized) {
            if (that.dropDownAppendTo !== null && event.relatedTarget) {
                if (that.contains(event.target)) {
                    const externalContainer = event.relatedTarget.closest('.lw-menu-drop-down');
    ...
    _mouseoutMouseoverHandler(event) {
        ...
        let closestItem = event.target.closest('lw-menu-item') || event.target.closest('lw-menu-items-group');

        if (that.enableShadowDOM) {
            closestItem = event.composedPath()[0].closest('lw-menu-item') || event.composedPath()[0].closest('lw-menu-items-group') || closestItem;
        }
        ...
        if (event.type === 'mouseover') {
            if (that.selectionMode === 'mouseenter' && that.mode !== 'tree' && !that._minimized) {
                if (closestItem instanceof LW.MenuItemsGroup && !closestItem.hasAttribute('hover')) {
                    that._selectionHandler(event, closestItem);
                }
                else if (closestItem instanceof LW.MenuItem) {
                    that._closeSubContainers(closestItem.level + 1);
                }
            }

            const closestDropDown = event.target.closest('.lw-menu-drop-down');
            ...
            if (that.mode !== 'tree' && !that._minimized) {
                ...
                if (closestItem instanceof LW.MenuItemsGroup &&
                    closestItem.container &&
                    !closestItem.container.$.hasClass('lw-visibility-hidden')) {
                    return;
                }
            }
    ...
    _moveDropDownsToExternalContainer() {
        ...
        for (let i = 0; i < that._containers.length; i++) {
            ...
            container.classList.add('lw-drop-down-repositioned');
    ...
    _moveDropDownsToMenu() {
        ...
        for (let i = 0; i < that._containers.length; i++) {
            ...
            container.classList.remove('lw-drop-down-repositioned');
    ...
    _processDataSource() {
        ...
        function processItem(item, parent) {
            ...
            if (Array.isArray(item[itemsMember]) && item[itemsMember].length > 0) {
                element = document.createElement('lw-' + that._element + '-items-group');
                ...
                element = document.createElement('lw-' + that._element + '-item');
    ...
    _processHTML(item, level, autoSort) {
        ...
        if (level > 1) {
            ...
            if (item instanceof LW.MenuItemsGroup) {
                ...
                arrowElement.className = 'lw-' + that._element + '-items-group-arrow';
            ...
            if (level === 2) {
                ...
                if (that._edgeMacFF) {
                    container.addEventListener('transitionend', function (event) {
                        if (event.target === this && this.$.hasClass('lw-visibility-hidden')) {
                            this.style.left = '';
                            this.style.top = '';
    ...
    _processUList() {
        ...
        while (menuItemsGroupOpeningTagRegex.test(innerHTML)) {
            const match = menuItemsGroupOpeningTagRegex.exec(innerHTML),
                content = '<lw-' + that._element + '-items-group>' + match[0].slice(4, match[0].length - 4);
        ...
        while (menuItemsGroupClosingTagRegex.test(innerHTML)) {
            const match = menuItemsGroupClosingTagRegex.exec(innerHTML),
                content = '</lw-' + that._element + '-items-group>';

            innerHTML = innerHTML.replace(match[0], content);
        }

        innerHTML = innerHTML.replace(/li>/g, 'lw-' + that._element + '-item>');
    ...
    _removeContainerFixedHeight() {
        ...
        for (let i = 0; i < that._containersFixedHeight.length; i++) {
            ...
            container.children[0].$.addClass('lw-hidden');
            container.children[2].$.addClass('lw-hidden');
            container.itemContainer.checkOverflow = true;
        }
    }
    ...
    _selectionHandler(event, closestItemsGroup) {
        ...
        if (closestItemsGroup === undefined) {
            ...
            const closestItem = target.closest('lw-' + that._element + '-item');
            ...
            closestItemsGroup = target.closest('lw-' + that._element + '-items-group');
    ...
    _setArrowDirection(arrowElement, level) {
        ...
        if (mode === 'tree' || that._minimized) {
            arrowElement.className = 'lw-' + that._element + '-items-group-arrow down lw-arrow-down';
            return;
        }
        ...
        if (that.dropDownPosition.slice(0, 7) !== 'overlay') {
            if (level === 2 && mode === 'horizontal') {
                if (that.dropDownPosition.slice(0, 3) !== 'top') {
                    arrowElement.className = 'lw-menu-items-group-arrow down lw-arrow-down';
                }
                else {
                    arrowElement.className = 'lw-menu-items-group-arrow up lw-arrow-up';
                }
            }
            else {
                arrowElement.className = 'lw-menu-items-group-arrow ' + (that.rightToLeft ? 'left lw-arrow-left' : 'right lw-arrow-right');
            }
        }
        else {
            arrowElement.className = 'lw-menu-items-group-arrow minus';
        }
    }
    ...
    /**
     * Sets the label of a lw-menu-item/lw-menu-items-group.
     */
    _setItemLabel(item, label) {
        const that = this,
            oldContext = that.context,
            labelElement = item.querySelector('.lw-menu-item-label-element');
    ...
    _setLabel(label, labelElement, item, labelSet) {
        ...
        if (potentialHTMLTemplate !== null && potentialHTMLTemplate.tagName.toLowerCase() === 'template') {
            ...
            if (item instanceof LW.MenuItem) {
            ...
            else if (LW.ListMenu && this instanceof LW.ListMenu) {
    ...
    _toggleItem(item) {
        ...
        if (that.checkboxes) {
            ...
            if (parent.checkable) {
                ...
                if (item instanceof LW.MenuItem && fireItemClick) {
    ...
    _transitionendHandlerCollapse() {
        ...
        container.menuItemsGroup.$.removeClass('lw-' + menu._element + '-items-group-expanded');
    ...
    /**
     * Updates the WAI-ARIA roles of lw-menu-items and lw-menu-items-groups.
     */
