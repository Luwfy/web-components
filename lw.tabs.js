 ...
 */
LW('lw-tab-item', class TabItem extends LW.ContentElement {
    ...
    /**
     * Appends a child node directly to the lw-tab-item's container.
     *
 ...
 */
LW('lw-tab-items-group', class TabItemsGroup extends LW.ContentElement {
 ...
 * Tabs custom element.
 */
LW('lw-tabs', class Tabs extends LW.BaseElement {
    // Tabs's properties.
    static get properties() {
        return {
            'addNewTab': {
    ...
    template() {
        return `<div id="container">
                    <div id="tabsHeaderSection" class="lw-header lw-tabs-header-section">
                        <div id="tabsHeaderItems" class="lw-tabs-header-items">
                            <lw-repeat-button id="scrollButtonNear" class="lw-tabs-scroll-button lw-nav-button lw-spin-button lw-hidden" animation="[[animation]]" unfocusable>
                                <div id="arrowNear" class="lw-arrow" aria-hidden="true"></div>
                            </lw-repeat-button>
                            <div id="tabStrip" class="lw-tab-strip"></div>
                            <lw-repeat-button id="scrollButtonFar" class="lw-tabs-scroll-button lw-nav-button lw-spin-button lw-hidden" animation="[[animation]]" unfocusable>
                                <div id="arrowFar" class="lw-arrow" aria-hidden="true"></div>
                            </lw-repeat-button>
                            <div id="dropDownButton" class="lw-drop-down-button" role="button" aria-expanded="false" aria-label="Open tab selection popup">
                                <div id="dropDownButtonArrow" class="lw-drop-down-button-arrow lw-unselectable" aria-hidden="true"></div>
                            </div>
                            <span id="tabSelectionBar" class="lw-tabs-selection-bar" role="presentation"></span>
                            <div id="resizeToken" class="lw-tabs-resize-token lw-hidden" role="presentation"></div>
                        </div>
                        <div id="tabHeaderControls" class="lw-tabs-header-controls"></div>
                    </div>
                    <div id="tabContentSection" class="lw-tabs-content-section">
                       <content></content>
                    </div>
                    <div id="dropDownButtonDropDown" class="lw-drop-down-button-drop-down lw-hidden"></div>
    ...
    ready() {
        ...
        that._flexWritingModeNoFullSupport = true; // !LW.Utilities.Core.Browser.Chrome;
    }
    ...
    /**
     * Appends a "lw-tab-item" node as the last tab of the Tabs.
     *
     * @param {HTMLElement} node The "lw-tab-item" node to append.
     */
    appendChild(node) {
        const that = this;
    ...
    /**
     * Returns the offset of the tab item container (lw-tab-item element) from the edge of the Tabs (lw-tabs element) where the tab strip is positioned.
     *
    ...
    insert(index, details) {
        ...
        if (details.node === undefined) {
            group = details.group;
            tab = document.createElement('lw-tab-item');
        ...
        tab.$.addClass('lw-visibility-hidden');
        ...
        if (that._groups.length === 0 && group === undefined) {
            ...
            if (group !== undefined && group !== null) {
                ...
                const lwTabItemsGroup = that.$.tabContentSection.querySelector('lw-tab-items-group[label="' + group + '"]');

                if (lwTabItemsGroup !== null) {
                    // group exists
                    tabDetails.lwTabItemsGroup = lwTabItemsGroup;
    ...
    /**
     * Inserts the specified "lw-tab-item" node before the reference "lw-tab-item" node.
     *
     * @param {HTMLElement} newNode The "lw-tab-item" node to insert.
     * @param {HTMLElement} referenceNode The "lw-tab-item" node before which newNode is inserted.
     */
    insertBefore(newNode, referenceNode) {
        ...
        if (!(newNode instanceof LW.TabItem) || referenceNode !== undefined && referenceNode !== null && !(referenceNode instanceof LW.TabItem)) {
    ...
    removeAt(index) {
        ...
        if (tabLabelContainer._lazyRemove) {
            tabLabelContainer.classList.add('lw-hidden');
        }
        else {
        ...
        if (tab.group !== null) {
            const lwTabItemsGroup = that.$.tabContentSection.querySelector('lw-tab-items-group[label="' + tab.group + '"]');

            if (lwTabItemsGroup.childElementCount === 0) {
                ...
                that.$.tabContentSection.removeChild(lwTabItemsGroup);
            }
        }
    ...
    /**
     * Removes a child "lw-tab-item" node.
     *
     * @param {HTMLElement} node The "lw-tab-item" node to remove.
     */
    removeChild(node) {
        const that = this;
    ...
    _addGroupContainer(label, labelSize) {
        ...
        groupContainer.className = 'lw-tab-group-container lw-unselectable';
        groupContainer.$ = LW.Utilities.Extend(groupContainer);
        ...
        labelTextWrapper.className = 'lw-tab-label-text-wrapper';
        ...
        // label
        labelTextContainer.className = 'lw-tab-label-text-container';
        labelTextContainer.innerHTML = label;

        // down arrow
        arrow.className = 'lw-tab-group-arrow';

        // dropdown
        dropDown.className = 'lw-tab-group-drop-down lw-hidden';
        dropDown.$ = LW.Utilities.Extend(dropDown);
    ...
    _addGroupElements(details) {
        ...
        if (groupIndex !== -1) {
            ...
            const groupContainers = that._addGroupContainer(group, details.lwTabItemsGroup.labelSize),
        ...
        if (that.selectedIndex === details.i) {
            ...
            groupLabel.$.addClass('lw-tab-group-selected');
        }
    }
    ...
    _addTabLabelContainer(tab, addNewTab) {
        ...
        tabLabelContainer.className = 'lw-tab-label-container lw-unselectable';
        labelTextWrapper.className = 'lw-tab-label-text-wrapper';
        ...
        // close button
        closeButton.className = 'lw-tab-close-button';
        ...
        if (addNewTab === undefined) {
            if (that.closeButtons) {
                if (that.closeButtonMode === 'default' && tab.closeButtonHidden ||
                    ...
                    closeButton.classList.add('lw-hidden');
                    tabLabelContainer.closeButtonEnabled = false;
                }
                else {
                    labelTextContainer.classList.add('lw-close-button-enabled');
            ...
            dropDownLabelContainer.className = 'lw-tab-label-container lw-unselectable';
        ...
        labelTextContainer.$ = LW.Utilities.Extend(labelTextContainer);
        closeButton.$ = LW.Utilities.Extend(closeButton);
        ...
        tabLabelContainer.$ = LW.Utilities.Extend(tabLabelContainer);
    ...
    _animateSelection(tab, oldTab, indexGreaterThanOldIndex) {
        ...
        if (tab === oldTab) {
            tab.$.hasClass('lw-visibility-hidden') ? tab.$.removeClass('lw-visibility-hidden') : tab.$.addClass('lw-visibility-hidden');
        ...
        if (indexGreaterThanOldIndex) {
            tab.$.addClass(side1);
            tab.$.removeClass('lw-visibility-hidden');

            oldTab.$.addClass('animate');
            tab.$.addClass('animate');

            oldTab.$.addClass(side2);
            oldTab.$.addClass('lw-visibility-hidden');
    ...
    _applyConditionalTabOverflow() {
        ...
        if (that._flexWritingModeNoFullSupport && that.tabLayout === 'shrink' && verticalPosition &&
            ...
            for (let i = 0; i < tabStripChildren.length; i++) {
                ...
                tabStripChildren[i].firstElementChild.firstElementChild.classList.add('lw-tab-label-text-container-full-height');
            }
    ...
    /**
     * lw-tab-item closeButtonVisibilityChange event handler.
     */
    _closeButtonVisibilityChangeHandler(event) {
        const that = this,
            index = (LW.EnableShadowDOM ? event.composedPath()[0] : event.target).index;
    ...
    _closeDropDownButtonDropDown() {
        ...
        if (that._dropDownButtonDropDownOpened) {
            ...
            that.$dropDownButtonDropDown.addClass('lw-hidden');
        ...
        if (that._openDropDown.rippleInProgress) {
            setTimeout(function () {
                if (that._openDropDown) {
                    ...
                    that._openDropDown.$.addClass('lw-hidden');
    ...
    _containerHandler(event) {
        ...
        if (that.disabled ||
            ...
            ((eventType === 'down' || eventType === 'up') && event.which !== 1 && !LW.Utilities.Core.isMobile)) {
            return;
        }
    ...
    _documentMoveHandler(event) {
        ...
        if (that._reordering) {
            ...
            if (that.tabLayout !== 'wrap' &&
                ...
                that.tabLayout === 'wrap' &&
                (event.originalEvent.target.closest('.lw-tab-label-container') !== null ||
                    event.originalEvent.target.closest('.lw-tab-group-container') !== null) &&
            ...
            that.$resizeToken.removeClass('lw-hidden');
        }
    }
    ...
    _documentUpHandler(event) {
        ...
        if (that._openDropDown && !that._openDropDown.rippleInProgress &&
            (target === document || target.closest('.lw-tab-group-container') !== that._openDropDown.groupContainer) &&
    ...
    _dropDownButtonHandler(target, event, dropDown) {
        ...
        if (dropDown) {
            if (target.classList.contains('lw-tab-label-container')) {
                switch (eventType) {
                    case 'down':
        ...
        if (eventType === 'down') {
            ...
            if (!that._dropDownButtonDropDownOpened) {
                ...
                that.$dropDownButtonDropDown.removeClass('lw-hidden');
    ...
    _fixTabStripNotChrome(tabStrip) {
        ...
        if (LW.Utilities.Core.Browser.Safari) {
            setTimeout(function () {
                tabStrip.parentElement.classList.add('lw-hidden');
                setTimeout(function () {
                    tabStrip.parentElement.classList.remove('lw-hidden');
                    recalcTabStripWidth();
                }, 10);
            }, 10);
        }
    ...
    _fixTabStripFlex(tabLayout) {
        ...
        if (!that._flexWritingModeNoFullSupport && that.tabTextOrientation === 'vertical' && (that.tabPosition === 'left' || that.tabPosition === 'right')) {
            setTimeout(function () {
                that.$tabsHeaderSection.addClass('lw-hidden');
                setTimeout(function () {
                    that.$tabsHeaderSection.removeClass('lw-hidden');
                    that._getTabCoordinates();
    ...
    _getCorrespondingCustomElement(currentItem) {
        if (currentItem.$.hasClass('lw-tab-label-container')) {
            return currentItem.tab;
        }
        else {
            return this.$.tabContentSection.querySelector('lw-tab-items-group[label="' + currentItem.group + '"]');
        }
    }
    ...
    _getTabs() {
        const that = this;
        let tabs = that.shadowRoot ? that.shadowRoot.querySelectorAll('lw-tab-item') : that.getElementsByTagName('lw-tab-item');
    ...
    _groupContainerHandler(currentElement, eventType) {
        ...
        if (!that._reordering && !that._swiping) {
            if (dropDown !== that._openDropDown && eventType === 'mouseover' && !currentElement.classList.contains('lw-tab-group-selected')) {
                currentElement.setAttribute('hover', '');
            }
            else if (eventType === 'mouseout') {
                currentElement.removeAttribute('hover');
        ...
        if (dropDown === that._openDropDown) {
            ...
            if (!currentElement.classList.contains('lw-tab-group-selected')) {
                currentElement.setAttribute('hover', '');
            }
        }
        else {
    ...
    _handlePosition(newPosition, oldPosition) {
        ...
        if (newPosition === 'top' || newPosition === 'bottom') {
            ...
            that.$arrowNear.removeClass('lw-arrow-up');
            that.$arrowNear.addClass('lw-arrow-left');
            that.$arrowFar.removeClass('lw-arrow-down');
            that.$arrowFar.addClass('lw-arrow-right');
            that.setAttribute('aria-orientation', 'horizontal');
            ...
            that.$arrowNear.removeClass('lw-arrow-left');
            that.$arrowNear.addClass('lw-arrow-up');
            that.$arrowFar.removeClass('lw-arrow-right');
            that.$arrowFar.addClass('lw-arrow-down');
    ...
    _hideCloseButton(tabLabelContainer) {
        tabLabelContainer.firstElementChild.children[0].$.removeClass('lw-close-button-enabled');
        tabLabelContainer.firstElementChild.children[1].$.addClass('lw-hidden');
    }

    /**
     * lw-tab-item indexChange event handler.
     */
    ...
    _insertAddNewTab() {
        ...
        tabLabelContainer.$.addClass('lw-add-new-tab');
    ...
    _insertIntoExistingGroup(details) {
        const that = this,
            lwTabItemsGroup = details.lwTabItemsGroup,
            tab = details.tab,
            tabLabelContainer = details.tabLabelContainer,
            group = details.group,
        ...
        index = Math.max(0, Math.min(index, lwTabItemsGroup.childElementCount));
        ...
        if (sibling) {
            ...
            overallIndex = lwTabItemsGroup.children[index - 1].index + 1;
        }

        lwTabItemsGroup.insertBefore(tab, sibling);
    ...
    _insertIntoNewGroup(details) {
        ...
        const newLWTabItemsGroup = document.createElement('lw-tab-items-group');

        newLWTabItemsGroup.appendChild(tab);
        that.$.tabContentSection.insertBefore(newLWTabItemsGroup, that.$.tabContentSection.children[index]);
        newLWTabItemsGroup.label = group;

        const previousSibling = newLWTabItemsGroup.previousElementSibling;
        let overallIndex = 0;

        if (previousSibling) {
            if (previousSibling instanceof LW.TabItem) {
        ...
        if (previous && previous.group !== null && next && groupOfNext !== null) {
            ...
            if (next) {
                if (groupOfNext !== null) {
                    ...
                    that.$.tabContentSection.insertBefore(tab, that.$.tabContentSection.querySelector('lw-tab-items-group[label="' + groupOfNext + '"]'));
                }
                else {
        ...
        switch (key) {
            ...
            case ' ':
                ...
                if (that._focusedItem) {
                    ...
                    const closestGroupContainer = that._focusedItem.tabLabelContainer.closest('.lw-tab-group-drop-down');
    ...
    _labelContainerHandler(currentElement, event, eventType) {
        ...
        if (eventType === 'down') {
            ...
            if (that.selectionMode === 'click' && currentElement !== that._addNewTab) {
                if (LW.Utilities.Core.isMobile && currentElement.tab.group === null) {
    ...
    /**
     * lw-tab-item labelSizeChange event handler.
     */
    _labelSizeChangeHandler(event) {
        ...
        if (event.target instanceof LW.TabItem) {
    ...
    */
    _openGroupDropDown(currentElement) {
        ...
        currentElement.dropDown.$.removeClass('lw-hidden');
    ...
    _orderTabs() {
        const that = this,
            groups = that.getElementsByTagName('lw-tab-items-group'),
            indexes = [], list = [];
        let unset = true,
            ambiguous = false;
    ...
    _populateTabStrip() {
        ...
        for (let i = 0; i < that._tabs.length; i++) {
            const tab = that._tabs[i],
                ...
                group = tabParentElement instanceof LW.TabItemsGroup ? tabParentElement.label : null;
            ...
            if (that.selectedIndex === i) {
                tabLabelContainer.$.addClass('lw-tab-selected');
            ...
            if (group === null) {
                ...
                that._addGroupElements({ documentFragment: documentFragment, groups: groups, groupLabels: groupLabels, i: i, group: group, tabLabelContainer: tabLabelContainer, lwTabItemsGroup: tabParentElement });
            }
    ...
    _positionTabSelectionBar(selectedTabLabelContainer, noAnimation) {
        ...
        if (LW.Utilities.Core.isMobile && that.hasAnimation && !noAnimation) {
            requestAnimationFrame(function () {
    ...
    _processDataSource() {
        ...
        function processItem(item, parent) {
            ...
            if (Array.isArray(item.items) && item.items.length > 0) {
                element = document.createElement('lw-tab-items-group');
                ...
                element = document.createElement('lw-tab-item');
    ...
    _removeInlineStyle() {
        ...
        if (that._inlineStyleTabStripChildren) {
            ...
            for (let i = 0; i < tabStripChildren.length; i++) {
                ...
                tabStripChildren[i].firstElementChild.firstElementChild.classList.remove('lw-tab-label-text-container-full-height');
            }
    ...
    _reorderTabsGrouping(firstTabIndex, secondTabIndex) {
        ...
        that._groupLabels = Array.from(that.$.tabStrip.getElementsByClassName('lw-tab-group-container'));
        that._groups = [];
        ...
        that._tabs = Array.from(that.$.tabContentSection.getElementsByTagName('lw-tab-item'));
        ...
        for (let i = 0; i < that.$.tabStrip.childElementCount; i++) {
            ...
            if (currentChild.$.hasClass('lw-tab-label-container') && currentChild !== that._addNewTab) {
                tabLabelContainers.push(currentChild);
            }
            else if (currentChild.$.hasClass('lw-tab-group-container')) {
    ...
    _resize(event) {
        ...
        that.$resizeToken.addClass('lw-hidden');
        that._applyTabOverflow();
    ...
    _ripple(event, target) {
        ...
        if (that.hasRippleAnimation) {
            ...
            LW.Utilities.Animation.Ripple.animate(rippleAgent, event.pageX, event.pageY);
    ...
    _scrollButtonClickHandler(event) {
        const that = this,
            tabStrip = that.$.tabStrip;
        let modifier = event.target.closest('lw-repeat-button') === that.$.scrollButtonNear ? -1 : 1;
    ...
    */
    _getScrollLeft(scrollLeft) {
        ...
        if (scrollLeft === undefined) {
            if (that.rightToLeft && LW.Utilities.Core.Browser.Chrome) {
    ...
    _select(index, fireEvent, oldIndex) {
        ...
        if (oldIndex !== null) {
            ...
            that._tabLabelContainers[oldIndex].$.removeClass('lw-tab-selected');
        ...
        if (!that.allowToggle || (index !== null && (arguments[2] !== undefined ? oldIndex : that.selectedIndex) !== index)) {
            ...
            that._focusedItem.tabLabelContainer.$.addClass('lw-tab-selected');
        ...
        if (tab && tab.group !== null) {
            if (that._selectedGroup) {
                that._selectedGroup.$.removeClass('lw-tab-group-selected');
            }

            if (that.selectedIndex !== null) {
                that._selectedGroup = that._groupLabels[that._groups.indexOf(tab.group)];
                that._selectedGroup.$.addClass('lw-tab-group-selected');
            }
        }
    ...
    _showCloseButton(tabLabelContainer) {
        if (tabLabelContainer.closeButtonEnabled) {
            tabLabelContainer.firstElementChild.children[0].$.addClass('lw-close-button-enabled');
            tabLabelContainer.firstElementChild.children[1].$.removeClass('lw-hidden');
        }
    }
    ...
    _tabStripHandler(target, event) {
        ...
        if (eventType === 'down' && LW.Utilities.Core.isMobile) {
        ...
        const scrollButton = target.closest('lw-repeat-button');
        ...
        if (target.classList.contains('lw-tab-close-button')) {
            // close button
            if (eventType === 'up') {
                if (target === that._downTarget) {
        ...
        const closestLabelContainer = target.closest('.lw-tab-label-container');
        ...
        const closestGroupContainer = target.closest('.lw-tab-group-container');
    ...
    _toggleNavigationElementsVisibility() {
        ...
        if (that.tabLayout === 'scroll' && that.overflow === 'scroll') {
            that.$scrollButtonNear[action]('lw-hidden');
            that.$scrollButtonFar[action]('lw-hidden');
        }
        else if (that.tabLayout === 'dropDown') {
            that.$dropDownButton[action]('lw-hidden');
        }
    }
    ...
    _validateInitialSelection() {
        ...
        for (let i = 0; i < that._tabs.length; i++) {
            ...
            if (selectedIndex === i) {
                ...
                currentTab.$.removeClass('lw-visibility-hidden');
            }
                ...
                if (selectedIndex === null) {
                    ...
                    currentTab.$.addClass('lw-visibility-hidden');
                }
            }
            else {
                currentTab.$.addClass('lw-visibility-hidden');
        ...
        if (that._tabs.length > 0 && selectedIndex === null && !that.allowToggle) {
            ...
            that._tabs[0].$.removeClass('lw-visibility-hidden');
        }
