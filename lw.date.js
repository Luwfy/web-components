...
LW.Utilities.Assign('TimeSpan', class TimeSpan {
    constructor() {
        const that = this;
    ...
    add(ts) {
        const result = this._ticks + ts._ticks,
            timeSpan = new LW.Utilities.TimeSpan(result);
    ...
    subtract(ts) {
        ...
        return new LW.Utilities.TimeSpan(result);
    }
    ...
    duration() {
        ...
        if (that._ticks >= 0) {
            return new LW.Utilities.TimeSpan(that._ticks);
        }
        else {
            return new LW.Utilities.TimeSpan(-that._ticks);
        }
    ...
    interval(value, scale) {
        ...
        return new LW.Utilities.TimeSpan(millis * this.ticksPerMillisecond);
    }
    ...
    fromTicks(value) {
        return new LW.Utilities.TimeSpan(value);
    }
    ...
    negate() {
        return new LW.Utilities.TimeSpan(-this._ticks);
    }
});

LW.Utilities.Assign('DateTime', class DateTime {
    constructor() {
        function getUTCDate(ISOString) {
            const parseResult = that.regexISOString.exec(ISOString);

            UTCDate = new LW.Utilities.DateTime(
    ...
    static validateDate(value, referenceValue, formatString) {
        if (value instanceof LW.Utilities.DateTime) {
            return value;
        }
        ...
        try {
            if (new RegExp(/^\d{1,2}\/\d{1,2}\/\d{4}(, \d{1,2}:\d{2}:\d{1,2} [A|P]M)?$/).test(value)) {
                ...
                if (!isNaN(constructedDate.getTime())) {
                    return new LW.Utilities.DateTime(constructedDate);
                }
            }
            ...
            if (typeof value === 'number') {
                return new LW.Utilities.DateTime(new Date(value));
            }
        }
        catch (error) {
            //
    ...
    static parseDateString(value, referenceValue, formatString) {
        ...
        if (!isNaN(parseAttemptDate.getTime())) {
            return new LW.Utilities.DateTime(parseAttemptDate);
        }
        ...
        if ((indexOfDate !== -1 || indexOfDateTime !== -1) && indexOfBracket !== -1) {
            ...
            if (validValue.trim() === '') {
                return new LW.Utilities.DateTime();
            }
            ...
            if (new RegExp(/(^(\d+)(\s*,\s*\d+)+(\s*,\s*[a-zA-Z ]+)?$)/g).test(validValue)) {
                ...
                validValue = new (Function.prototype.bind.apply(LW.Utilities.DateTime, validValue));

                return validValue;
            }
        }
        else {
            const parseAttemptDateTime = new LW.Utilities.DateTime(),
                parseAttempt = parseAttemptDateTime.parseDate(value, formatString);

            if (parseAttempt) {
                validValue = new LW.Utilities.DateTime(parseAttempt);
        ...
        if (!isNaN(validValue)) {
            ...
            return new LW.Utilities.DateTime(validValue);
        }
        ...
        try {
            ...
            if (splitValue.length === 2) {
                ...
                validValue = new (Function.prototype.bind.apply(LW.Utilities.DateTime, splitValue));
            }
            else {
                validValue = new LW.Utilities.DateTime(validValue);
            }
        }
        catch (error) {
    ...
    static fromFullTimeStamp(timeStamp) {
        ...
        const dateTime = new LW.Utilities.DateTime(parseFloat(timeStamp.slice(0, timeStamp.length - 21)));
    ...
    subtract(date) {
        ...
        return new LW.Utilities.TimeSpan(that.dateData * that.ticksPerMillisecond - date.dateData * that.ticksPerMillisecond);
    }
    ...
    toString(format, calendar) {
        ...
        if (LW.Utilities.DateTime.cache && LW.Utilities.DateTime.cache[formatStamp]) {
            return LW.Utilities.DateTime.cache[formatStamp];
        }
        ...
        if (!LW.Utilities.DateTime.cache) {
            LW.Utilities.DateTime.cache = new Array();
        }
        LW.Utilities.DateTime.cache[formatStamp] = result;
        return result;
    }
    ...
    add(value, scale, createNew) {
        ...
        if (scale === undefined) {
            ...
            const newDate = new LW.Utilities.DateTime((ms + parseInt(value._ticks / that.ticksPerMillisecond)));
        ...
        const newDate = new LW.Utilities.DateTime(ms + millis);
        that.copyTimeZone(newDate);
        that.copySmallTimePartValues(newDate);
        return newDate;
    }
    ...
    clone() {
        const that = this,
            date = new LW.Utilities.DateTime(that.dateData);
    ...
    clearTime() {
        const that = this,
            ...
            year = that.year(),
            day = that.day(),
            newDate = new LW.Utilities.DateTime(year, month, day, 0, 0, 0, 0);
    ...
    addMonths(months, createNew) {
        ...
        const newDate = new LW.Utilities.DateTime((that.dateToMS(y, m, d) + that.internalMS() % that.millisPerDay));
        that.copyTimeZone(newDate);
        that.copySmallTimePartValues(newDate);
        return newDate;
    }
    ...
    date() {
        const that = this,
            ...
            year = that.year(),
            day = that.day(),
            newDate = new LW.Utilities.DateTime(year, month, day);
    ...
    toTimeZone(timeZone) {
        ...
        const date = this.toDate(tz),
            lwDate = new LW.Utilities.DateTime(date);

        lwDate.timeZone = tz;
        lwDate.timeZoneOffset = matches[0].offset;
        lwDate.timeZoneOffsetHours = matches[0].offsetHours;

        that.copySmallTimePartValues(lwDate);
    ...
    getTimeStamp() {
        const that = this,
            timeStamp = new LW.Utilities.BigNumber(that.dateData.toString() + that.getFirstNDigitsOfPrecision(undefined, true));
