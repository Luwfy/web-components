 ...
 */
LW('lw-slider', class Slider extends LW.Tank {
    /**
     * Slider's properties.
     */
    ...
    */
    ...
    template() {
        const template =
            `<div id="container" class="lw-container" role="presentation">
                <div id="scaleNear" class="lw-scale lw-scale-near" aria-hidden="true"></div>
                <div id="trackContainer" class="lw-track-container" role="presentation">
                    <lw-repeat-button id="leftButton" class="lw-spin-button" animation="[[animation]]" unfocusable>
                        <div id="leftArrow" class="lw-arrow" aria-hidden="true"></div>
                    </lw-repeat-button>
                    <div id="track" class="lw-track" role="presentation">
                        <div id="fill" class="lw-value" role="presentation"></div>
                        <div id="trackTicksContainer" class="lw-track-ticks-container lw-hidden" role="presentation"></div>
                        <div id="thumb" class="lw-thumb" role="slider">
                            <span id="thumbLabel" class="lw-thumb-label" role="presentation"></span>
                            <div id="tooltip" class="lw-tooltip" role="tooltip">
                                <div id="tooltipContent" class="lw-tooltip-content lw-unselectable" role="presentation"></div>
                            </div>
                        </div>
                        <div id="secondThumb" class="lw-thumb" role="slider">
                            <span id="secondThumbLabel" class="lw-thumb-label" role="presentation"></span>
                            <div id="secondTooltip" class="lw-tooltip" role="tooltip">
                                <div id="secondTooltipContent" class="lw-tooltip-content lw-unselectable" role="presentation"></div>
                            </div>
                        </div>
                    </div>
                    <lw-repeat-button id="rightButton" class="lw-spin-button" animation="[[animation]]" unfocusable>
                        <div id="rightArrow" class="lw-arrow" aria-hidden="true"></div>
                    </lw-repeat-button>
                </div>
                <div id="scaleFar" class="lw-scale lw-scale-far" aria-hidden="true"></div>
    ...
    _createElement() {
        ...
        const valuesHandler = that._valuesHandler = that.rangeSlider ? new LW.Utilities.SliderMultipleValueHandler(that) : new LW.Utilities.SliderSingleValueHandler(that);
        ...
        if (value !== undefined) {
            if (that.mode === 'date') {
                if (!that.rangeSlider) {
                    ...
                    value[0] = LW.Utilities.DateTime.validateDate(value[0]);
                    value[1] = LW.Utilities.DateTime.validateDate(value[1]);
    ...
    getOptimalSize() {
        ...
        if (that.orientation === 'horizontal') {
            ...
            if (that.scalePosition === 'near' || that.scalePosition === 'both') {
                ...
                labels = that.$.scaleNear.getElementsByClassName('lw-label');
                firstLabel = labels[0];
            ...
            if (that.scalePosition === 'far' || that.scalePosition === 'both') {
                optimalSize += largestLabelSize;
                labels = that.$.scaleFar.getElementsByClassName('lw-label');
                firstLabel = labels[0];
                lastLabel = labels[labels.length - 1];
        ...
        if (key !== 'values' && value != oldValue || key === 'values' && (value[0] != oldValue[0] || value[1] !== oldValue[1])) {
            switch (key) {
                ...
                case 'orientation':
                    ...
                    if (value === 'horizontal') {
                        that.$leftArrow.removeClass('lw-arrow-up');
                        that.$rightArrow.removeClass('lw-arrow-down');
                        that.$leftArrow.addClass('lw-arrow-left');
                        that.$rightArrow.addClass('lw-arrow-right');
                    }
                    else {
                        that.$leftArrow.removeClass('lw-arrow-left');
                        that.$rightArrow.removeClass('lw-arrow-right');
                        that.$leftArrow.addClass('lw-arrow-up');
                        that.$rightArrow.addClass('lw-arrow-down');
                    }
                ...
                case 'rangeSlider':
                    if (value) {
                        ...
                        valuesHandler = that._valuesHandler = new LW.Utilities.SliderMultipleValueHandler(that);
                    }
                    else {
                        ...
                        valuesHandler = that._valuesHandler = new LW.Utilities.SliderSingleValueHandler(that);
                ...
                case 'showButtons':
                    if (value) {
                        that.$leftButton.removeClass('lw-hidden');
                        that.$rightButton.removeClass('lw-hidden');
                    }
                    else {
                        that.$leftButton.addClass('lw-hidden');
                        that.$rightButton.addClass('lw-hidden');
                    }
                    that._setTicksAndInterval();
                ...
                case 'showTooltip':
                    ...
                    if (!value) {
                        ...
                        that.$secondTooltip.addClass('lw-hidden');
                    }
    ...
    /**
     * Adds the class "lw-moved-thumb" to the moved thumb.
     */
    _addMovedThumbClass() {
        const that = this;
        ...
        that._movedThumb.$.addClass('lw-moved-thumb');

        if (that._movedThumb === that.$.thumb) {
            that.$secondThumb.removeClass('lw-moved-thumb');
        }
        else {
            that.$thumb.removeClass('lw-moved-thumb');
        }
    }
    ...
    _trackDownHandler(event) {
        ...
        if (that.showTooltip) {
            that._movedTooltip.removeClass('lw-hidden');
        }
    }
    ...
    _documentUpHandler(event, canceled) {
        ...
        if (that.showTooltip) {
            that._movedTooltip.addClass('lw-hidden');
        }
    ...
    */
    ...
    _moveThumbBasedOnCoordinates(event, checkBoundaries, changeValue) {
        ...
        if (LW.Utilities.Core.isMobile && event.originalEvent) {
 ...
 */
LW.Utilities.Assign('SliderSingleValueHandler', class SliderSingleValueHandler {
    constructor(context) {
        ...
        if (context.mode === 'date') {
            context._getEventValue = function (aria, value) {
                ...
                if (value !== undefined) {
                    result = LW.Utilities.DateTime.fromFullTimeStamp(value);
                }
                else {
    ...
    moveThumbBasedOnValue(value, triggerEvent, noUpdate) {
        ...
        if (noUpdate !== true) {
            ...
            if (context.mode === 'date') {
                context._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(updatedValue);
            }
    ...
    validate(initialValidation, programmaticValue, programmaticValueIsSet) {
        ...
        if (initialValidation) {
            ...
            if (context.mode === 'date') {
                context._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(value);
            }
 ...
 */
LW.Utilities.Assign('SliderMultipleValueHandler', class SliderMultipleValueHandler {
    constructor(context) {
        this.context = context;
        ...
        if (context.mode === 'date') {
            context._getEventValue = function (aria, value, index) {
                ...
                if (value !== undefined) {
                    result[index] = LW.Utilities.DateTime.fromFullTimeStamp(value);
                }
    ...
    keyIncrementDecrement(action, changedIndex) {
        ...
        if (context.mode === 'date') {
            ...
            if (newValue.compare(context._drawMin) === -1) {
                return new LW.Utilities.BigNumber(context._drawMin);
            }

            if (newValue.compare(context._drawMax) === 1) {
                return new LW.Utilities.BigNumber(context._drawMax);
            }
    ...
    moveThumbBasedOnValue(value, changedIndex, noUpdate) {
        ...
        if (noUpdate !== true) {
            ...
            if (context.mode === 'date') {
                context._valueDate = [
                    LW.Utilities.DateTime.fromFullTimeStamp(actualValues[0]),
                    LW.Utilities.DateTime.fromFullTimeStamp(actualValues[1])
                ];
            }
        ...
        if (updateTooltip) {
            ...
            if (numericProcessor.compare(numericProcessor.createDescriptor(newValue), numericProcessor.createDescriptor(oldValue))) {
                ...
                if (changeValue) {
                    ...
                    if (context.mode === 'date') {
                        context._valueDate = [
                            LW.Utilities.DateTime.fromFullTimeStamp(updatedValues[0]),
                            LW.Utilities.DateTime.fromFullTimeStamp(updatedValues[1])
                        ];
                    }
    ...
    validate(initialValidation, programmaticValue, programmaticValueIsSet) {
        ...
        if (initialValidation) {
            ...
            if (context.mode === 'date') {
                context._valueDate = [
                    LW.Utilities.DateTime.fromFullTimeStamp(validNumbers[0]),
                    LW.Utilities.DateTime.fromFullTimeStamp(validNumbers[1])
                ];
            }
