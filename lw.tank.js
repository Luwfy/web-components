 ...
 * Tank custom element.
 */
LW('lw-tank', class Tank extends LW.BaseElement {
    /**
     * Tank's properties.
     */
    ...
    static get requires() {
        return {
            ...
            'LW.Utilities.TickIntervalHandler': 'lw.tickintervalhandler.js'
        }
    }
    ...
    */
    ...
    template() {
        return `<div id="container" class="lw-container" role="presentation">
                    <div id="scaleNear" class="lw-scale lw-scale-near" role="presentation" aria-hidden="true"></div>
                    <div id="track" class="lw-track" role="presentation">
                        <div id="fill" class="lw-value" role="presentation">
                            <div id="bubbleContainer" class="lw-bubble-container" role="presentation"></div>
                            <div id="tooltip" class="lw-tooltip" role="tooltip"><div id="tooltipContent" class="lw-tooltip-content lw-unselectable" role="presentation"></div></div>
                            <div id="thumb" class="lw-thumb">
                                <div class="lw-thumb-label-container">
                                    <span id="thumbLabel" class="lw-thumb-label"></span>
                                </div>
                            </div>
                        </div>
                        <div id="trackTicksContainer" class="lw-track-ticks-container lw-hidden" role="presentation"></div>
                    </div>
                    <div id="scaleFar" class="lw-scale lw-scale-far" role="presentation" aria-hidden="true"></div>
    ...
    _createElement() {
        ...
        that._numericProcessor = new LW.Utilities.NumericProcessor(that, 'scaleType');
    ...
    val(value) {
        ...
        if (value !== undefined) {
            if (that.mode === 'date') {
                let newValue = LW.Utilities.DateTime.validateDate(value);
    ...
    */
        ...
        switch (key) {
            ...
            case 'max': {
                if (that.mode === 'date') {
                    ...
                    that[key] = LW.Utilities.DateTime.validateDate(value).getTimeStamp();
                }
            ...
            case 'ticksPosition':
                if (value === 'scale') {
                    that.$trackTicksContainer.addClass('lw-hidden');
                    that.$.trackTicksContainer.innerHTML = '';
                }
                else {
                    that.$trackTicksContainer.removeClass('lw-hidden');
                }
                that._numericProcessor.addTicksAndLabels();
                break;
            case 'customInterval':
    ...
    _setInitialComponentDisplay() {
        ...
        switch (that.scalePosition) {
            case 'near':
                that.$scaleNear.removeClass('lw-hidden');
                that.$scaleFar.addClass('lw-hidden');
                break;
            case 'far':
                that.$scaleNear.addClass('lw-hidden');
                that.$scaleFar.removeClass('lw-hidden');
                break;
            case 'both':
                that.$scaleFar.removeClass('lw-hidden');
                that.$scaleNear.removeClass('lw-hidden');
        ...
        that.$tooltip.addClass('lw-hidden');
    ...
    **/
    ...
    _validateMinMax(validatedProperty, initialValidation, oldValue) {
        ...
        function validator(param, oldValue) {
            ...
            if (validateCondition) {
                if (oldValue) {
                    that._numberRenderer = new LW.Utilities.NumberRenderer(oldValue);
                ...
                that._numberRenderer = new LW.Utilities.NumberRenderer(value);
    ...
    _calculateDateInterval(majorTicksInterval) {
        ...
        let part = 'year',
            bigNumberTimePart = new LW.Utilities.BigNumber('31536000000000000000000000000000'),
            difference = bigNumberTimePart.subtract(majorTicksInterval).abs(),
            range = new LW.Utilities.BigNumber(that.min).subtract(that.max).abs(),
        ...
        for (let timePart in timeParts) {
            if (timeParts.hasOwnProperty(timePart)) {
                const currentBigNumberTimePart = new LW.Utilities.BigNumber(timeParts[timePart]),
        ...
        if (calculatedInterval) {
            customTicks = [new LW.Utilities.BigNumber(that.min)];
        ...
        switch (part) {
            case 'year':
                if (calculatedInterval) {
                    for (let i = that._minDate.year() + toAdd; i < that._maxDate.year(); i += toAdd) {
                        customTicks.push(new LW.Utilities.BigNumber(new LW.Utilities.DateTime(i, 1, 1).getTimeStamp()));
                    }
                }
            ...
            case 'month':
                if (calculatedInterval) {
                    for (let i = new LW.Utilities.DateTime(that._minDate.year(), that._minDate.month() + toAdd, 1);
                        i.compare(that._maxDate) === -1; i.addMonths(toAdd, false)) {
                        customTicks.push(new LW.Utilities.BigNumber(i.getTimeStamp()));
                    }
                }
            ...
            case 'day':
                if (calculatedInterval) {
                    for (let i = new LW.Utilities.DateTime(that._minDate.year(), that._minDate.month(), that._minDate.day() + toAdd);
            ...
            case 'hour':
                if (calculatedInterval) {
                    for (let i = new LW.Utilities.DateTime(that._minDate.year(), that._minDate.month(), that._minDate.day(), that._minDate.hour() + toAdd);
            ...
            case 'minute':
                if (calculatedInterval) {
                    for (let i = new LW.Utilities.DateTime(that._minDate.year(), that._minDate.month(), that._minDate.day(), that._minDate.hour(), that._minDate.minute() + toAdd);
        ...
        if (calculatedInterval) {
            if (customTicks[customTicks.length - 1].compare(that.max) === -1) {
                customTicks.push(new LW.Utilities.BigNumber(that.max));
            }
    ...
    _formatNumber(value) {
        ...
        if (that.mode === 'date') {
            const date = LW.Utilities.DateTime.fromFullTimeStamp(value);
    ...
    */
    _formatLabel(labelValue, unselectableUnit) {
        ...
        if (that.labelFormatFunction) {
            if (that.mode === 'date') {
                labelValue = LW.Utilities.DateTime.fromFullTimeStamp(labelValue);
            }
        ...
        that._numberRenderer = new LW.Utilities.NumberRenderer(renderedLabel);
        if (that.showUnit) {
            if (unselectableUnit !== false) {
                renderedLabel += ' <span class="lw-unselectable">' + that.unit + '</span>';
            }
            else {
    ...
    _trackDownHandler(event) {
        ...
        if (that.disabled || that.readonly || (!LW.Utilities.Core.isMobile && event.button !== 0)) {
            return;
        }
        ...
        that.$track.addClass('lw-dragged');

        if (that.showTooltip) {
            that.$tooltip.removeClass('lw-hidden');
        }
    }
    ...
    _documentUpHandler(event) {
        ...
        that.$track.removeClass('lw-dragged');
    ...
    **/
    _getSize(largestLabelSize, properties) {
        ...
        function calcScaleSize(selector, distance) {
            const labels = selector.getElementsByClassName('lw-label');
    ...
    _getRange() {
        ...
        if (that.scaleType === 'floatingPoint') {
            ...
            that._range = new LW.Utilities.BigNumber(that._drawMax).subtract(that._drawMin).toString();
        }
    }
    ...
    */
    _updateScaleWidth(longestLabelSize) {
        ...
        let scaleSize = scaleCoeficient + longestLabelSize,
            ...
            variablesUsed = Boolean(trackComputedStyle.getPropertyValue('--lw-tank-scale-size'));

        if (variablesUsed) {
            that.$.container.style.setProperty('--lw-tank-scale-size', scaleSize + 'px');
        }
        else {
    ...
    _validate(initialValidation, programmaticValue, coerced, programmaticValueIsSet) {
        ...
        if (initialValidation) {
            ...
            if (that.mode === 'numeric') {
                ...
                that._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(actualValue);
    ...
    */
    _setTicksAndInterval() {
        ...
        that._tickIntervalHandler = new LW.Utilities.TickIntervalHandler(that, minLabel, maxLabel, 'lw-label', that._settings.size, that.scaleType === 'integer', that.logarithmicScale);
    ...
    */
    ...
    _keyIncrementDecrement(action, preValue) {
        ...
        if (calculatePreValue) {
            preValue = that.logarithmicScale ? new LW.Utilities.BigNumber(that._drawValue) : that._drawValue;
        }
        ...
        if (that.validation === 'interaction' &&
            ...
            that._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(that._drawValue);
        }

        that._valueDate[that._dateIncrementMethod](actionCoefficient * parseFloat(that.interval), false);
        that._drawValue = new LW.Utilities.BigNumber(that._valueDate.getTimeStamp());

        if (that._drawValue.compare(that._drawMin) === -1) {
            that._drawValue = new LW.Utilities.BigNumber(that._drawMin);
            that._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(that._drawValue);
        }

        if (that._drawValue.compare(that._drawMax) === 1) {
            that._drawValue = new LW.Utilities.BigNumber(that._drawMax);
            that._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(that._drawValue);
        }
    ...
    */
    _keydownHandler(event) {
        ...
        if (that.scaleType === 'floatingPoint') {
            ...
            let testValue = new LW.Utilities.BigNumber(that._drawValue);
    ...
    **/
    ...
    _handleDateScale() {
        const that = this,
            dateTime = LW.Utilities.DateTime;

        if (!dateTime) {
            that.error(that.localize('missingReference', { elementType: that.nodeName.toLowerCase(), files: 'lw.date.js' }));
        }
        ...
        Object.defineProperty(that, propertyName, {
            ...
            set(value) {
                function replacer(key, value) {
                    if (value instanceof LW.Utilities.BigNumber) {
