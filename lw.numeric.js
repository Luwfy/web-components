 ...
 */
LW.Utilities.Assign('NumericProcessor', class NumericProcessor {
    constructor(context, numericFormatProperty) {
        switch (context[numericFormatProperty]) {
            case 'integer':
                return new LW.Utilities.IntegerNumericProcessor(context, numericFormatProperty);
            case 'floatingPoint':
                return new LW.Utilities.DecimalNumericProcessor(context, numericFormatProperty);
            case 'complex':
                return new LW.Utilities.ComplexNumericProcessor(context, numericFormatProperty);
        }
    }
});
 ...
 */
LW.Utilities.Assign('BaseNumericProcessor', class BaseNumericProcessor {
    constructor(context, numericFormatProperty) {
        const that = this;
        ...
        if (window.NIComplex) {
            ...
            that.complexConstructor = LW.Utilities.Complex;
        }
    }
    ...
    _createMeasureLabel() {
        ...
        measureLabel.className = 'lw-label';
    ...
    _addMajorTickAndLabel(htmlValue, labelSize, plot, value, middle) {
        ...
        if (that.nodeName.toLowerCase() === 'lw-tank' || that._intervalHasChanged) {
        ...
        currentTick = '<div style="' + leftOrTop + ': ' + offset + 'px;" class="lw-tick"></div>';
        ...
        if (plot !== false) {
            ...
            currentLabel += '<div class="lw-label' + (middle ? ' lw-label-middle' : '') + '" style="' + leftOrTop + ': ' + labelOffset + 'px;">' + htmlValue + '</div>';
        }
    ...
    drawGaugeLogarithmicScaleMinorTicks(majorTickValues, majorStep, drawMinor) {
        ...
        if (majorStep instanceof LW.Utilities.BigNumber) {
    ...
    addCustomTicks() {
        ...
        if (that.nodeName.toLowerCase() === 'lw-tank') {
 ...
 */
LW.Utilities.Assign('IntegerNumericProcessor', class IntegerNumericProcessor extends LW.Utilities.BaseNumericProcessor {
    constructor(context, numericFormatProperty) {
    ...
    createDescriptor(initialValue, supportsENotation, validateConstruction, validateMinMax, discardRadix) {
        ...
        if (initialValue.constructor !== LW.Utilities.BigNumber) {
            let radix = !discardRadix && that._radixNumber ? that._radixNumber : 10;

            if (radix === 10 && supportsENotation && initialValue.constructor !== LW.Utilities.BigNumber && this.isENotation(initialValue)) {
                if (LW.Utilities.BigNumber.bigIntSupport) {
                    initialValue = new LW.Utilities.BigNumber(Math.round(initialValue));
                }
                else {
                    initialValue = new LW.Utilities.NumberRenderer(initialValue).largeExponentialToDecimal();
                }
            }
            if (that._toBigNumberDecimal) {
                ...
                returnValue = new LW.Utilities.BigNumber(initialValue);
            }
        }
        else {
            returnValue = new LW.Utilities.BigNumber(initialValue);
    ...
    */
    round(value) {
        ...
        if (value instanceof window.LW.Utilities.BigNumber && typeof value._d === 'bigint') {
            return value;
        }
        else if (value instanceof window.LW.Utilities.BigNumber === false ||
            !context._wordLengthNumber || context._wordLengthNumber < 64) {
            return new LW.Utilities.BigNumber(Math.round(value.toString()));
        }
    ...
    validateMinMax(validateMin, validateMax) {
        ...
        let defaultMin = this.defaultMins[that.wordLength],
            defaultBigMin = new LW.Utilities.BigNumber(defaultMin),
            defaultMax = this.defaultMaxs[that.wordLength],
            defaultBigMax = new LW.Utilities.BigNumber(defaultMax),
            numericScale = that.mode !== 'date';

        if (that._numberRenderer === undefined) {
            that._numberRenderer = new LW.Utilities.NumberRenderer();
        }
        ...
        if (validateMin && numericScale) {
            ...
            let currentBigMin = this.round(new LW.Utilities.BigNumber(that.min));
        ...
        if (validateMax && numericScale) {
            ...
            let currentBigMax = this.round(new LW.Utilities.BigNumber(that.max));
        ...
        if (!numericScale) {
            that._minObject = new LW.Utilities.BigNumber(that.min);
            that._maxObject = new LW.Utilities.BigNumber(that.max);
        }
        ...
        if (!numericScale) {
            that._minDate = LW.Utilities.DateTime.fromFullTimeStamp(that.min);
            that._maxDate = LW.Utilities.DateTime.fromFullTimeStamp(that.max);
        }
    }
     ...
     */
    valueToPx(value) {
        const ignored = LW.Utilities.BigNumber.ignoreBigIntNativeSupport;

        LW.Utilities.BigNumber.ignoreBigIntNativeSupport = true;

        const that = this.context,
            lengthRangeRatio = new LW.Utilities.BigNumber(that._measurements.trackLength).divide(new LW.Utilities.BigNumber(that._range));

        let result;

        if (that._normalLayout) {
            const drawMin = that._drawMin instanceof LW.Utilities.BigNumber ? that._drawMin : new LW.Utilities.BigNumber(that._drawMin);

            value = new LW.Utilities.BigNumber(value);
            result = parseFloat((lengthRangeRatio.multiply(value.subtract(drawMin))).toString());
        }
        else {
            const drawMax = that._drawMax instanceof LW.Utilities.BigNumber ? that._drawMax : new LW.Utilities.BigNumber(that._drawMax);

            result = parseFloat(this.round(((drawMax.subtract(value)).multiply(lengthRangeRatio))).toString());
        }

        LW.Utilities.BigNumber.ignoreBigIntNativeSupport = ignored;
        return result;
    }
    ...
    */
    pxToValue(px) {
        const that = this.context,
            ignored = LW.Utilities.BigNumber.ignoreBigIntNativeSupport;
        let result;
        ...
        if (that.logarithmicScale) {
            ...
            return new LW.Utilities.BigNumber(Math.round(Math.pow(10, power)));
        }
    ...
    compare(initialValue, otherValue, thorough) {
        ...
        if (initialValue.constructor !== LW.Utilities.BigNumber) {
            initialValue = new LW.Utilities.BigNumber(initialValue);
        }
    ...
    incrementDecrement(initialValue, operation, stepObject) {
        ...
        if (operation === 'add') {
            ...
            if (that._drawMax !== undefined) {
                return returnValue.compare(that._drawMax) > 0 ? new LW.Utilities.BigNumber(that._drawMax) : returnValue;
            }
        }
        else {
            returnValue = initialValue.subtract(stepObject);
            if (that._drawMin !== undefined) {
                return returnValue.compare(that._drawMin) < 0 ? new LW.Utilities.BigNumber(that._drawMin) : returnValue;
            }
        }
        return returnValue;
    }
    ...
    render(initialValue, ignoreRadixNumber) {
        ...
        if (!context.scientificNotation && ignoreRadixNumber === true) {
            return new LW.Utilities.NumberRenderer(new LW.Utilities.BigNumber(initialValue)).bigNumberToExponent(context.significantDigits);
        }
        ...
        if (context.scientificNotation && ignoreRadixNumber === true) {
            returnValue = new LW.Utilities.NumberRenderer(returnValue).toScientific(context.significantDigits, context.precisionDigits);
        }
    ...
    addTicksAndLabels() {
        ...
        const that = this.context,
            ...
            tickscount = this.round(new LW.Utilities.BigNumber(that._range).divide(ticksFrequency)),
            ticksDistance = trackLength / tickscount,
            min = new LW.Utilities.BigNumber(that._drawMin),
            max = new LW.Utilities.BigNumber(that._drawMax);
    ...
    */
    addMiddleMajorTicks(tickscount, ticksDistance, distanceFromFirstToSecond, distanceModifier, normalLayout, ticksFrequency) {
        ...
        for (let i = 1; i < tickscount; i++) {
            ...
            if (normalLayout) {
                value = ticksFrequency.multiply(i).add(distanceModifier.add(new LW.Utilities.BigNumber(that._drawMin)));
            }
            else {
                value = new LW.Utilities.BigNumber(that._drawMax).subtract(distanceModifier).subtract(ticksFrequency.multiply(i));
    ...
    addMinorTicks(normalLayout) {
        function addMinorTick(i) {
            if (tickValues.indexOf(i) === -1 && i % minorTicksInterval === 0) {
                minorTicks += '<div style="' + leftOrTop + ': ' + that._numericProcessor.valueToPx(i) + 'px;" class="lw-tick lw-tick-minor"></div>';
            }
        }
            ...
            for (let j = that._drawMax; j > 0; j = j - 1) {
                ...
                for (let i = range; i > 0; i = (i - modifier)) {
                    if ((i < that.max) && (i > that.min)) {
                        let value = new LW.Utilities.BigNumber(Math.log10(i));

                        if (((value % 1 === 0) && ticksOnPowerOf10) || !ticksOnPowerOf10) {
                            minorTicks += '<div style="' + leftOrTop + ': ' + that._numericProcessor.valueToPx(value) + 'px;" class="lw-tick lw-tick-minor"></div>';
                        }
                    }
                }
            }
    ...
    validateCustomTicks() {
        ...
        for (let i = 0; i < that.customTicks.length; i++) {
            ...
            if (numericScale) {
                ...
                currentTick = LW.Utilities.DateTime.validateDate(currentTick).getTimeStamp();
            }
    ...
    addGaugeTicksAndLabels() {
        ...
        const that = this.context,
            ...
            bigDrawMin = new LW.Utilities.BigNumber(that._drawMin),
            bigDrawMax = new LW.Utilities.BigNumber(that._drawMax);
        ...
        for (let i = new LW.Utilities.BigNumber(second); i.compare(bigDrawMin) !== -1; i = i.subtract(minorStep)) {
            firstMinTick = i;
        }
    ...
    updateToolTipAndValue(value, oldValue, changeValue) {
        ...
        value = value instanceof LW.Utilities.BigNumber ? value : new LW.Utilities.BigNumber(value);
        ...
        if (value.compare(oldValue) !== 0 && changeValue) {
            if (that.mode === 'numeric') {
                ...
                that._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(stringifiedValue);
                that.value = value;
    ...
    validateInterval(interval) {
        ...
        that._validInterval = new LW.Utilities.BigNumber(interval);
        that._validInterval = this.round(that._validInterval);

        if (that._validInterval.compare(0) <= 0) {
            that._validInterval = new LW.Utilities.BigNumber(1);
        }
    ...
    getCoercedValue(value, useDrawVariables, logarithmicGauge) {
        ...
        if (that.customInterval) {
            ...
            if (normalScale || logarithmicGauge) {
                ...
                closestValue = new LW.Utilities.BigNumber(closestValue);
            }
        ...
        if (that.mode === 'date') {
            if (that._dateIncrementMethod === 'addYears') {
                ...
                interval = new LW.Utilities.BigNumber(that._dateIntervalNumber).multiply(interval);
            }
        }

        if (useDrawVariables !== false) {
            minValue = new LW.Utilities.BigNumber(that._drawMin);
            maxValue = new LW.Utilities.BigNumber(that._drawMax);
        }
        else {
            minValue = new LW.Utilities.BigNumber(that.min);
            maxValue = new LW.Utilities.BigNumber(that.max);
        }
    ...
    coerceMonth(value) {
        const that = this.context,
            ...
            date = LW.Utilities.DateTime.fromFullTimeStamp(value),
            year = date.year(),
            month = date.month(),
            day = date.day(),
        ...
        that._drawValue = new LW.Utilities.BigNumber(result.getTimeStamp());
    ...
    coerceYear(value) {
        const that = this.context,
            ...
            updatedValueConstructorParameters = LW.Utilities.DateTime.getConstructorParameters(that._minDate);
        let year = date.year();
        ...
        const valueDate = new (Function.prototype.bind.apply(LW.Utilities.DateTime, updatedValueConstructorParameters));
    ...
    updateValue(value) {
        ...
        if (that.mode === 'numeric') {
            ...
            oldValue = LW.Utilities.DateTime.fromFullTimeStamp(oldValue);
            that._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(value);
            that.value = value;
            value = that._valueDate;
            valueDetail = value;
    ...
    getValuePerPx(range, pxRange) {
        ...
        const result = new LW.Utilities.BigNumber(range).divide(pxRange);
    ...
    restrictValue(values) {
        if (values[1].constructor === LW.Utilities.BigNumber) {
    ...
    getAngleByValue(value, calculateDrawValue, returnDegrees) {
        ...
        if (value instanceof LW.Utilities.BigNumber === false) {
            value = new LW.Utilities.BigNumber(value);
        }

        if (calculateDrawValue !== false && that.logarithmicScale) {
            value = new LW.Utilities.BigNumber(Math.log10(value.toString()));
        }
    ...
    getValueByAngle(angle) {
        ...
        value = new LW.Utilities.BigNumber((minuendAngle - subtrahendAngle) / that._angleDifference).multiply(that._range).add(that._drawMin);

        if (that.logarithmicScale) {
            value = new LW.Utilities.BigNumber(Math.pow(10, value.toString()));
        }
        ...
        LW.Utilities.BigNumber.ignoreBigIntNativeSupport = ignored;
        return new LW.Utilities.BigNumber(result);
    }
     ...
     */
    updateGaugeValue(newValue) {
        if (newValue instanceof LW.Utilities.BigNumber === false) {
        ...
        if (that.mode === 'numeric') {
            ...
            that._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(newValue);
            that.value = newValue;
        }
    ...
    validateColorRange(value) {
        ...
        if (that.mode === 'numeric') {
            ...
            value = LW.Utilities.DateTime.validateDate(value);
            value = value.getTimeStamp();
        }

        const bigMin = new LW.Utilities.BigNumber(that.min),
            bigMax = new LW.Utilities.BigNumber(that.max);
    ...
    getAngleRangeCoefficient() {
        ...
        LW.Utilities.BigNumber.ignoreBigIntNativeSupport = true;
        that._angleRangeCoefficient = new LW.Utilities.BigNumber(that._angleDifference).divide(that._range);
        LW.Utilities.BigNumber.ignoreBigIntNativeSupport = ignored;
    }
});
 ...
 */
LW.Utilities.Assign('DecimalNumericProcessor', class DecimalNumericProcessor extends LW.Utilities.BaseNumericProcessor {
    constructor(context, numericFormatProperty) {
        super(context, numericFormatProperty);
        ...
        if (typeof moduloCoefficient === 'undefined') {
            ...
            if ((dividend < -1 || dividend > 1) && (divisor < -1 || divisor > 1 || divisor === 1)) {
                if (dividend % 1 === 0 && divisor % 1 === 0) {
                    ...
                    const result = sign * parseFloat(new LW.Utilities.BigNumber(dividend).mod(divisor).toString());
    ...
    render(initialValue) {
        ...
        if (checkSpecialRegex) {
            ...
            const numberRenderer = new LW.Utilities.NumberRenderer(initialValue);
 ...
 */
LW.Utilities.Assign('ComplexNumericProcessor', class ComplexNumericProcessor extends LW.Utilities.BaseNumericProcessor {
    constructor(context, numericFormatProperty) {
        super(context, numericFormatProperty);
        this.context = context;
    }
    ...
    render(initialValue) {
        ...
        if (this.context._regexSpecial.nonNumericValue.test(initialValue) === false) {
            ...
            const realPartRenderer = new LW.Utilities.NumberRenderer(realPart),
                imaginaryPartRenderer = new LW.Utilities.NumberRenderer(imaginaryPart);
    ...
    compareComplexNumbers(left, right) {
        if ((left instanceof this.complexConstructor === false) || (right instanceof this.complexConstructor === false)) {
            // if at least one of the numbers is not an LW.Utilities.Complex object, the numbers are different
            return -1;
        }
...
LW.Utilities.Assign('NumberRenderer', class NumberRenderer {
    ...
    largeExponentialToDecimal(exponential) {
        ...
        let stringExponential = exponential.toString().toLowerCase(),
            ...
            mantissa = new LW.Utilities.BigNumber(stringExponential.slice(0, indexOfE)),
        ...
        let exponent = stringExponential.slice(indexOfE + 2),
            bigTen = new LW.Utilities.BigNumber(10),
    ...
    bigNumberToExponent(significantDigits, forceExponentialResult) {
        ...
        if (value.constructor !== LW.Utilities.BigNumber) {
            value = new LW.Utilities.BigNumber(value);
        }
        ...
        if (numberLength <= 10) {
            ...
            if (forceExponentialResult) {
                ...
                return new LW.Utilities.NumberRenderer(parseFloat(numericString)).toDigits(significantDigits);
            }
        }
        else {
            // 64-bit
            if (significantDigits >= numberLength && !forceExponentialResult) {
                ...
                if (nextDigit >= 5) {
                    numericString = new LW.Utilities.BigNumber(numericString.slice(0, significantDigits)).add(1).toString();
                }
    ...
    toScientific() {
        ...
        let power = parseInt(exponentialValue.slice(indexOfE + 1), 10),
            ...
            coefficientRenderer = new LW.Utilities.NumberRenderer(coefficient);
        ...
        if (power < 0) {
            ...
            if (test.charAt(test.length - 1) === '0') {
                ...
                if (i) {
                    ...
                    LW.Utilities.BigNumber.ignoreBigIntNativeSupport = true;
                    coefficient = parseFloat(new LW.Utilities.BigNumber(coefficient).multiply(Math.pow(10, i)).toString());
                    LW.Utilities.BigNumber.ignoreBigIntNativeSupport = ignored;
                }
        ...
        if (remainderPower > 0) {
            ...
            LW.Utilities.BigNumber.ignoreBigIntNativeSupport = true;
            coefficient = parseFloat(new LW.Utilities.BigNumber(coefficient).multiply(Math.pow(10, remainderPower)).toString());
            LW.Utilities.BigNumber.ignoreBigIntNativeSupport = ignored;
        }
    ...
    bigNumberToScientific() {
        ...
        if (arguments[0] !== null) {
            ...
            if (parseFloat(stringRepresentation.slice(arguments[0], arguments[0] + 1)) >= 5) {
                scientificValue = new LW.Utilities.BigNumber(scientificValue).add(1).toString();
            }
    ...
    formatNumber(number, formatSpecifier, wordLength) {
        ...
        let numericProcessor = new LW.Utilities.NumericProcessor(that, 'inputFormat'),
        ...
        switch (format) {
            ...
            case 'x': {
                ...
                if (that.inputFormat !== 'integer') {
                    that.inputFormat = 'integer';
                    numericProcessor = new LW.Utilities.NumericProcessor(that, 'inputFormat');
