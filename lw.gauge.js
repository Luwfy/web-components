 ...
 */
LW('lw-gauge', class Gauge extends LW.Tank {
    /**
     * Gauge's properties.
     */
    ...
    */
    ...
    template() {
        const template =
            `<div id="container" role="presentation">
                <div id="svgContainer" class="lw-svg-container" role="presentation" aria-hidden="true"></div>
                <div class="lw-digital-display-container" role="presentation">
                    <lw-numeric-text-box id="digitalDisplay"
                                            class="lw-digital-display"
    ...
    attached() {
        ...
        if (that._trackListener) {
            that._trackListener = new LW.Utilities.InputEvents(that._track);
        ...
        if (that._fillListener) {
            that._fillListener = new LW.Utilities.InputEvents(that._fill);
        ...
        if (that._lineListener) {
            that._lineListener = new LW.Utilities.InputEvents(that._line);
    ...
    _createElement() {
        ...
        that._draw = new LW.Utilities.Draw(that.$.svgContainer);
    ...
    _addAnalogDisplay() {
        ...
        if (that.analogDisplayType === 'needle') {
            ...
            that._centralCircle = draw.circle(radius, radius, centralCircleRadius, { 'class': 'lw-needle-central-circle' });
            ...
            // track
            that._track = draw.pieslice(radius, radius, distance - measurements.trackWidth, distance, that.startAngle, that.endAngle, 0, { 'class': 'lw-track' });
            that._trackListener = new LW.Utilities.InputEvents(that._track);
            that._trackListener.down(function (event) {
                that._SVGElementDownHandler(event);
            });
    ...
    _downHandler(event, targetIsTrack) {
        ...
        if (that.analogDisplayType !== 'needle' && !targetIsTrack || that.disabled || that.readonly ||
            ...
            (!LW.Utilities.Core.isMobile && (('buttons' in event && event.buttons !== 1) || event.which !== 1))) {
    ...
    _animate(oldValue, newValue) {
        ...
        if (newValue > oldValue) {
            getNextValue = function () {
                return Math.min(LW.Utilities.Animation.Easings.easeInOutSine(i, oldDrawValue, total, totalIterations), newDrawValue);
            }
        }
        else {
            getNextValue = function () {
                return Math.max(2 * oldDrawValue - LW.Utilities.Animation.Easings.easeInOutSine(i, oldDrawValue, total, totalIterations), newDrawValue);
            }
        }
    ...
    _drawFill(update, value) {
        ...
        if (that.analogDisplayType === 'fill') {
            ...
            if (update) {
                ...
                that._fill = that._draw.pieslice(radius, radius, distance - measurements.trackWidth, distance, startAngle, endAngle, 0, { 'class': 'lw-value' });
                that._fillListener = new LW.Utilities.InputEvents(that._fill);
                that._fillListener.down(function (event) {
                    that._SVGElementDownHandler(event);
                });
            ...
            if (update) {
                ...
                that._line = that._draw.line(x1, y1, x2, y2, { 'class': 'lw-line' });
                that._lineListener = new LW.Utilities.InputEvents(that._line);
                that._lineListener.down(function (event) {
                    that._SVGElementDownHandler(event);
                });
    ...
    _drawLabel(angle, value, distance, middle) {
        const that = this,
            ...
            stylingObj = {
                'class': 'lw-label' + (middle !== false ? ' lw-label-middle' : ''),
    ...
    _drawNeedle(update, value) {
        ...
        if (!that.drawNeedle) {
            ...
            if (update) {
                ...
                that._needle = that._draw.path(points, { 'class': 'lw-needle' });
            }
        }
        else {
    ...
    _drawRanges() {
        ...
        for (let i = 0; i < ranges.length; i += 1) {
            ...
            const range = that._draw.pieslice(radius, radius, distance - rangeSize, distance, numericProcessor.getAngleByValue(validEndValue, true, true), numericProcessor.getAngleByValue(validStartValue, true, true), 0, { 'class': 'lw-range ' + currentRange.className });
    ...
    _keydownHandler(event) {
        ...
        if (that.mode === 'numeric') {
            ...
            if (that.value.toString() !== stringValue) {
                that._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(stringValue);
    ...
    _validateValue(value, oldValue) {
        ...
        if (that.mode === 'numeric') {
            ...
            that._valueDate = LW.Utilities.DateTime.fromFullTimeStamp(stringValueNoRangeValidation);
