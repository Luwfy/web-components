 ...
 */
LW('lw-time-picker', class TimePicker extends LW.BaseElement {
    /**
    ...
    template() {
        const template =
            `<div id="container" role="presentation">
                <div id="header" class="lw-header lw-unselectable" role="heading" aria-level="1">
                    <div id="hourMinuteContainer" class="lw-hour-minute-container">
                        <div id="hourContainer" class="lw-hour-container" role="button"></div>
                        <div role="presentation">:</div>
                        <div id="minuteContainer" class="lw-minute-container" role="button"></div>
                    </div>
                    <div id="ampmContainer" class="lw-am-pm-container" role="presentation">
                        <div id="amContainer" class="lw-am-container" role="button">AM</div>
                        <div id="pmContainer" class="lw-pm-container" role="button">PM</div>
                    </div>
                </div>
                <div id="main" class="lw-main-container">
                    <div id="svgContainer" class="lw-svg-container">
                        <div id="picker" class="lw-svg-picker" role="slider"></div>
                    </div>
    ...
    propertyChangedHandler(key, oldValue, value) {
        ...
        switch (key) {
            ...
            case 'format': {
                ...
                if (value === '12-hour') {
                    that.$ampmContainer.removeClass('lw-hidden');
                    ...
                    that.$ampmContainer.addClass('lw-hidden');
                }
    ...
    _applyInitialSettings() {
        ...
        if (that.format === '12-hour') {
            if (hours < 12) {
                ...
                that._ampm = 'pm';
                that.$pmContainer.addClass('lw-selected');
        ...
        if (that.selection === 'hour') {
            that.$hourContainer.addClass('lw-selected');
            that.$.picker.setAttribute('aria-valuenow', hours);
        }
        else {
    ...
    _changeToHourSelection() {
        ...
        that.$hourContainer.addClass('lw-selected');
        that.$minuteContainer.removeClass('lw-selected');
    ...
    _changeToMinuteSelection() {
        ...
        that.$hourContainer.removeClass('lw-selected');
        that.$minuteContainer.addClass('lw-selected');
    ...
    _createElement() {
        ...
        that._numericProcessor = new LW.Utilities.DecimalNumericProcessor(that);
        that._draw = new LW.Utilities.Draw(that.$.picker);
    ...
    _drawArrowSVG(update, value) {
        ...
        if (update) {
            ...
            that._arrow = that._draw.path(arrowBodyPoints, { 'class': 'lw-needle' });
            that._head = that._draw.circle(that._headCenter.x, that._headCenter.y, that._largestLabelSize, { 'class': 'lw-needle-central-circle' });
            that._headRect = that._head.getBoundingClientRect();
        }
    }
    ...
    _drawLabel(angle, value, distance) {
        const that = this,
            ...
            stylingObj = {
                'class': 'lw-label lw-unselectable',
    ...
    _headerClickHandler(event) {
        ...
        switch (event.target) {
            ...
            case that.$.amContainer:
                if (!that.$amContainer.hasClass('lw-selected')) {
                    that._selectAmPm('am');
            ...
            case that.$.pmContainer:
                if (!that.$pmContainer.hasClass('lw-selected')) {
                    that._selectAmPm('pm');
    ...
    _highlightLabel(value) {
        ...
        if (that._highlightedLabel) {
            ...
            that._highlightedLabel.classList.remove('lw-selected');
        ...
        const labelAtValue = that.$.picker.querySelector('.lw-label[value="' + roundedValue + '"]');

        if (labelAtValue && (roundedValue === value || that._overlapsLabel(labelAtValue))) {
            that._highlightedLabel = labelAtValue;
            labelAtValue.classList.add('lw-selected');
        }
    }
        ...
        if (that.$.header.contains(activeElement) && (key === 'Enter' || key === ' ')) {
            ...
            if (coefficient) {
                ...
                if (that.selection === 'hour') {
                    ...
                    if (that.format === '12-hour') {
                        if (hours >= 12 && that.$amContainer.hasClass('lw-selected')) {
                            hours -= 12;
                        }
                        else if (hours < 12 && that.$pmContainer.hasClass('lw-selected')) {
                            hours += 12;
                        }
                    }
    ...
    _pickerDownHandler(event) {
        ...
        if (that.disabled || that.readonly || !LW.Utilities.Core.isMobile && event.which !== 1) {
            return;
        }
    ...
    _renderSVG() {
        ...
        that._centralCircle = that._draw.circle(that._measurements.radius, that._measurements.radius, 4, { 'class': 'lw-needle-central-circle' });
    ...
    _selectAmPm(which) {
        ...
        if (which === 'am') {
            that._ampm = 'am';
            that.$pmContainer.removeClass('lw-selected');
            that.$amContainer.addClass('lw-selected');
        }
        else {
            that._ampm = 'pm';
            that.$amContainer.removeClass('lw-selected');
            that.$pmContainer.addClass('lw-selected');
        }
    }
