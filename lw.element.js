...
(function () {
    ...
    class Ripple {
        static animate(element, left, top, callback) {
            ...
            if (target.getElementsByClassName('lw-ripple').length === 0) {
                const span = document.createElement('span');

                span.classList.add('lw-ripple');
                ...
                if (window[namespace].EnableShadowDOM && target.enableShadowDOM && target.isInShadowDOM !== true) {
                    ...
                    if (target.shadowRoot.host.shadowRoot.querySelector('.lw-ripple')) {
            ...
            if (window[namespace].EnableShadowDOM && target.shadowRoot) {
                ripple = target.shadowRoot.host.shadowRoot.querySelector('.lw-ripple');
            }
            else {
                ripple = target.getElementsByClassName('lw-ripple')[0];
            }
            ...
            ripple.innerHTML = '';
            ripple.classList.remove('lw-animate');
            ...
            ripple.classList.add('lw-animate');
    ...
    class Core {
        ...
        static getScriptLocation() {
            ...
            const location = (function () {
                ...
                if (error.fileName) {
                    return error.fileName.replace('/lw.element.js', '');
                }
                ...
                return link.replace('/lw.element.js', '');
            })();
    ...
    class InputEvents {
        constructor(target) {
            ...
            // that.target === document ? '' : that.target.id || that.target.getAttribute('lw-id');
        ...
        listen(eventType) {
            ...
            if (eventType === 'resize') {
                if (!that.target.resizeTrigger && that.target !== document && that.target !== window && that.target.hasResizeObserver !== false) {
                    ...
                    container.className = 'lw-resize-trigger-container';
                    container.innerHTML =
                        '<div class="lw-resize-trigger-container">' +
                        '<div class="lw-resize-trigger"></div>' +
                        '</div>' +
                        '<div class="lw-resize-trigger-container">' +
                        '<div class="lw-resize-trigger-shrink"></div>' +
                        '</div>';
        ...
        listen() {
            ...
            that.inputEvents['down'](function (event) {
                ...
                const originalTarget = event.originalEvent.target,
                    target = originalTarget && originalTarget.closest ? originalTarget.closest('lw-scroll-bar') : undefined;
    ...
    /**
     * This is a base class for LW Elements. It extends HTMLElement.
     */
    class BaseElement extends HTMLElement {
        ...
        getBindings(node, ownerMap) {
            ...
            if (node.getAttribute) {
                map.nodeId = node.getAttribute('lw-id');
        ...
        _addRemovePropertyBinding(hostPropertyName, targetPropertyName, targetElement, removeBinding, parentElement) {
            ...
            const id = targetElement.getAttribute('lw-id');
            ...
            for (let i = 0; i < htmlElement.attributes.length; i++) {
                ...
                if (attributeName.indexOf('(') >= 0) {
                    ...
                    if (that && !that.dataContext) {
                        that.templateListeners[htmlElement.getAttribute('lw-id') + '.' + eventName] = attributeValue;
        ...
        updateBoundNodes(propertyName) {
            ...
            if (that.detachedChildren.length > 0) {
                for (let i = 0; i < that.detachedChildren.length; i++) {
                    ...
                    const lwId = node.getAttribute('lw-id');

                    const getBindings = function (bindings) {
                        if (bindings.nodeId === lwId) {
                            return bindings;
                        }

                        for (let index in bindings.children) {
                            const node = bindings.children[index];
                            const attribute = node.getAttribute ? node.getAttribute('lw-id') : '';
                        ...
                        if (node.getAttribute && that.bindings.mapping) {
                            ...
                            if (bindings) {
                                for (let mapping in bindings.mapping) {
                                    const childNode = element.querySelector('[lw-id="' + mapping + '"]');
        ...
        updateBoundMappedNodes() {
            ...
            for (let mapping in bindings.mapping) {
                let childNode = element.querySelector('[lw-id="' + mapping + '"]');

                if (element.shadowRoot) {
                    childNode = element.querySelector('[id="' + mapping + '"]');

                    if (!childNode) {
                        childNode = element.shadowRoot.querySelector('[id="' + mapping + '"]') || element.shadowRoot.querySelector('[lw-id="' + mapping + '"]');
                    }
                }
                ...
                if (childNode) {
                    ...
                    childNode = detachedChildNode.querySelector('[lw-id="' + mapping + '"]');
            ...
            if (!detached) {
                for (let child = node.firstChild; child; child = child.nextSibling) {
                    ...
                    if (child.getAttribute) {
                        const childId = child.getAttribute('lw-id');
                ...
                if (!boundProperty.ready) {
                    ...
                    if (boundProperty.twoWay) {
                        ...
                        if (boundProperty.name.indexOf('::') >= 0) {
                            ...
                            that['$' + node.getAttribute('lw-id')].listen(eventName, function () {
                        ...
                        if (node.$ && node.$.isCustomElement) {
                            ...
                            that['$' + node.getAttribute('lw-id')].listen(attributeName + '-changed', function (event) {
        ...
        get classNamesMap() {
            return {
                animation: 'lw-animate',
                rippleAnimation: 'lw-ripple'
            }
        }
        ...
        addThemeClass() {
            ...
            if (that.theme !== '') {
                that.classList.add('lw-' + that.theme);
            }
        }
        ...
        _renderShadowRoot() {
            ...
            if (that.shadowRoot) {
                ...
                for (let i = 0; i < that.classList.length; i++) {
                    ...
                    if (className === 'lw-element-init' || className === 'lw-element' || className === 'lw-hidden' || className === 'lw-visibility-hidden') {
                        continue;
                    }
        ...
        ready() {
            ...
            if (that.visibility === 'collapsed') {
                that.classList.add('lw-hidden');
            }
            else if (that.visibility === 'hidden') {
                that.classList.add('lw-visibility-hidden');
            }
            ...
            if (that.shadowRoot && LW(that._selector)) {
                if (LW(that._selector).styleUrls) {
                ...
                if (LW(that._selector).styles) {
                    const scopedStyle = document.createElement('style');
                    scopedStyle.innerHTML = LW(that._selector).styles;
                    that.shadowRoot.insertBefore(scopedStyle, that.shadowRoot.firstChild);
                }
            }

            if (LW(that._selector) && LW(that._selector).ready) {
                LW(that._selector).ready();
            }
        }
        ...
        logLicense() {
            ...
            console.log('****************************************************************************************************************');
            console.log('****************************************************************************************************************');
            console.log('*LW HTML Elements License Key Not Found.');
        ...
        initProperties() {
            ...
            if (LW(that._selector) && LW(that._selector).properties) {
                that._initProperties = LW(that._selector).properties;
            }
            ...
            if (that._initProperties) {
                ...
                for (let i = 0; i < propertyNames.length; i++) {
                    ...
                    if (propertyValue.constructor === LW.ObservableArray || propertyValue instanceof LW.ObservableArray) {
        ...
        setup() {
            ...
            that.complete = function () {
                ...
                if (!that.wait) {
                    if (!that.classList.contains('lw-async')) {
            ...
            let templateNodes = [].slice.call(that.querySelectorAll('[lw-id]')).concat(children);

            if (window[namespace].EnableShadowDOM && that.isInShadowDOM !== true) {
                templateNodes = [].slice.call(that.shadowRoot.querySelectorAll('[lw-id]')).concat(children);
            }
        ...
        attached() {
            ...
            if (LW(that._selector) && LW(that._selector).attached) {
                LW(that._selector).attached();
            }
        }
        ...
        detached() {
            ...
            if (LW(that._selector) && LW(that._selector).detached) {
                LW(that._selector).detached();
            }
        }
        ...
        propertyChangedHandler(propertyName, oldValue, newValue) {
            ...
            if (propertyName === 'theme') {
                if (oldValue !== '') {
                    that.classList.remove('lw-' + oldValue);
                }

                if (newValue !== '') {
                    that.classList.add('lw-' + newValue);
                }
            }
            ...
            if (propertyName === 'visibility') {
                if (oldValue === 'collapsed') {
                    ...
                    that.classList.remove('lw-visibility-hidden');
                }
        ...
        _handleListeners(action) {
                ...
                for (let listener in listeners) {
                    ...
                    if (path[1]) {
                        ...
                        if (path[0] === 'document') {
                            ...
                            eventType = eventType + '.' + that.lwId + '_' + that.parents.length;
                        }
                    }
                    else if (that.lwId) {
                        eventType = eventType + '.' + that.lwId;
                    }
            ...
            if (LW(that._selector) && LW(that._selector).properties) {
                processListeners(LW(that._selector).listeners);
            }
        }
        ...
        applyTemplate() {
            ...
            for (let i = 0; i < templateNodes.length; i += 1) {
                ...
                templateNode.setAttribute('lw-id', templateNode.id);
            ...
            that.$root.addClass('lw-container');
            ...
            if (that.$.template) {
                ...
                templateContainer.classList.add('lw-template-container');
        ...
        addModule(module, subclasses) {
            ...
            if (subclasses) {
                for (let tagName in LW.Elements.tagNames) {
                    const element = LW.Elements.tagNames[tagName];
        ...
        defineElement() {
            ...
            proto._initElement = function () {
                ...
                that.lwId = (Date.now().toString(36) + Math.random().toString(36).substr(2, 5)).toUpperCase();
    ...
    class BaseCustomElement extends BaseElement {
        ...
        _setupShadowRoot() {
            ...
            that.classList.add('lw-element-init');
            ...
            const render = (element) => {
                ...
                element.setup();
                element.classList.remove('lw-element-init');
            }
            ...
            if (document.adoptedStyleSheets) {
                if (!window[namespace].AdoptedStyleSheets) {
                    ...
                    let url = Utilities.Core.getScriptLocation() + '/styles/lw.default.css';
        ...
        connectedCallback() {
            ...
            const updateVisibility = function () {
                that.classList.remove('lw-element-init');
            }
    ...
    class ObservableArray {
        ...
        concat() {
            ...
            return new LW.ObservableArray(result);
        }
    ...
    function Init(selector, element) {
        ...
        const defineProperties = function (properties, propertyPath) {
            ...
            for (let i = 0; i < propertyNames.length; i++) {
                ...
                if (propertyValue && typeof propertyValue === 'object' && LW.DataAdapter && propertyValue instanceof LW.DataAdapter) {
                    continue;
                }
    ...
    window.addEventListener('load', function () {
        ...
        for (let tagName in tagNames) {
            ...
            for (let j = 0; j < elements.length; j++) {
                ...
                element.classList.add('lw-element-ready');
            }
        ...
        if (customElements.length > 0) {
            ...
            for (let j = 0; j < customElements.length; j++) {
                ...
                if (tagName === 'lwItem') {
                    customElement = new window['lwListItem'](element);
                }
                else {
    ...
    window[namespace]('lw-base-element', window[namespace].BaseElement);
    /**
    * Content element.
    */
    window[namespace]('lw-content-element', class ContentElement extends window[namespace].BaseElement {
        static get properties() {
            return {
                'content': {
                    type: 'any',
    ...
    window[namespace]('lw-scroll-viewer', class ScrollViewer extends window[namespace].ContentElement {
        static get properties() {
            return {
        ...
        template() {
            return `<div id="container" class="lw-container" role="presentation">
                        <div id="scrollViewerContainer" class="lw-scroll-viewer-container" role="presentation">
                            <div id="scrollViewerContentContainer" inner-h-t-m-l=\'[[innerHTML]]\' class="lw-scroll-viewer-content-container" role="presentation">
                                <content></content>
                            </div>
                        </div>
                        <lw-scroll-bar id="verticalScrollBar" animation="[[animation]]" disabled="[[disabled]]" right-to-left="[[rightToLeft]]" orientation="vertical"></lw-scroll-bar>
                        <lw-scroll-bar id="horizontalScrollBar" animation="[[animation]]" disabled="[[disabled]]" right-to-left="[[rightToLeft]]"></lw-scroll-bar>
                    </div>`;
        }
        ...
        appendChild(node) {
            ...
            if (!that.isCompleted || (node.classList && node.classList.contains('lw-resize-trigger-container'))) {
                const args = Array.prototype.slice.call(arguments, 2);
        ...
        get computedHorizontalScrollBarVisibility() {
            ...
            if (that._scrollView && that._scrollView.hScrollBar) {
                return !that._scrollView.hScrollBar.$.hasClass('lw-hidden');
            }
        ...
        get computedVerticalScrollBarVisibility() {
            ...
            if (that._scrollView && that._scrollView.vScrollBar) {
                return !that._scrollView.vScrollBar.$.hasClass('lw-hidden');
            }
        ...
        refreshScrollBarsVisibility() {
            ...
            if (that.scrollWidth > 0) {
                that._scrollView.hScrollBar.$.removeClass('lw-hidden');
            }
            ...
            if (that.scrollHeight > 0) {
                that._scrollView.vScrollBar.$.removeClass('lw-hidden');
            }
    ...
    window[namespace].Utilities.Assign('PositionDetection', class PositionDetection {
        ...
        getDropDownParent(init) {
            ...
            if (init) {
                ...
                dropDown.$.addClass('lw-drop-down-repositioned');
            }
        ...
        dropDownAppendToChangedHandler() {
            ...
            if (that._dropDownParent === null) {
                ...
                dropDown.$.removeClass('lw-drop-down-repositioned');
            }
            else {
                that._dropDownParent.appendChild(dropDown);
                dropDown.$.addClass('lw-drop-down-repositioned');
        ...
        placeOverlay() {
            ...
            overlay.classList.add('lw-drop-down-overlay');
