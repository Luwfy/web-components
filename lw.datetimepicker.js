 ...
 */
LW('lw-date-time-picker', class DateTimePicker extends LW.BaseElement {
    /**
    ...
    static get properties() {
        return {
            ...
            'interval': {
                value: new LW.Utilities.TimeSpan(0, 0, 1),
                type: 'any'
            },
            'label': {
                value: '',
            ...
            'max': {
                value: new LW.Utilities.DateTime(3001, 1, 1),
                type: 'any'
            },
            'messages': {
                value: {
            ...
            'min': {
                value: new LW.Utilities.DateTime(1600, 1, 1),
                type: 'any'
            },
            'name': {
                value: '',
            ...
            'value': {
                ...
                value: new LW.Utilities.DateTime(),
                type: 'any'
            },
            'weekNumbers': {
                value: false,
    ...
    template() {
        return `<div id="container" role="presentation">
                    <span id="label" class="lw-label">[[label]]</span>
                    <div id="content" class="lw-content">
                        <input id="input" class="lw-input lw-date-time-input" type="text" readonly="[[readonly]]" disabled="[[disabled]]" placeholder="[[placeholder]]" name="[[name]]" aria-label="[[placeholder]]" />
                        <div id="spinButtonsContainer" class="lw-spin-buttons-container" role="presentation">
                            <lw-repeat-button initial-delay="[[spinButtonsInitialDelay]]" animation="[[animation]]" delay="[[spinButtonsDelay]]" readonly="[[readonly]]"  unfocusable id="upButton" right-to-left="[[rightToLeft]]" class="lw-spin-button" aria-label="Increment">
                                <div class="lw-arrow lw-arrow-up" aria-hidden="true"></div>
                            </lw-repeat-button>
                            <lw-repeat-button initial-delay="[[spinButtonsInitialDelay]]" animation="[[animation]]" delay="[[spinButtonsDelay]]" readonly="[[readonly]]" unfocusable id="downButton" right-to-left="[[rightToLeft]]" class="lw-spin-button" aria-label="Decrement">
                                <div class="lw-arrow lw-arrow-down" aria-hidden="true"></div>
                            </lw-repeat-button>
                        </div>
                        <div id="calendarButton" class="lw-drop-down-button lw-calendar-button" role="button" aria-haspopup="dialog" aria-label="Toggle popup"></div>
                        <div id="dropDownContainer" class="lw-drop-down lw-date-time-drop-down lw-drop-down-container lw-visibility-hidden" role="dialog">
                            <div id="dropDownHeader" class="lw-drop-down-header lw-hidden" role="heading" aria-level="1">
                                <div id="selectDate" class="lw-selected" role="button"></div>
                                <div id="selectTime" role="button"></div>
                            </div>
                            <div id="dropDownContent" class="lw-drop-down-content" role="presentation">
                                <lw-calendar id="calendarDropDown" class="lw-hidden"
                                              animation="[[animation]]"
                                              ...
                                              weeks="[[weeks]]"></lw-calendar>
                            </div>
                        </div>
                    </div>
                    <span id="hint" class="lw-hint lw-hidden">[[hint]]</span>
                </div>`;
    }
    ...
    ready() {
        ...
        that._defaultFooterTemplate.innerHTML =
            `<div class="lw-date-time-picker-footer">
                <div class="lw-footer-component lw-footer-component-hour" role="presentation">
                    <input type="text" class="lw-hour-element" aria-label="Hours" />
                    <span role="presentation">
                        <lw-repeat-button initial-delay="0" delay="75" aria-label="Increment hours">
                            <span class="lw-arrow lw-arrow-up" aria-hidden="true"></span>
                        </lw-repeat-button>
                        <lw-repeat-button  initial-delay="0" delay="75" aria-label="Decrement hours">
                            <span class="lw-arrow lw-arrow-down" aria-hidden="true"></span>
                        </lw-repeat-button>
                    </span>
                </div>
                <div class="lw-footer-component lw-footer-component-minute" role="presentation">
                    <input class="lw-minute-element" aria-label="Minutes" />
                    <span role="presentation">
                        <lw-repeat-button initial-delay="0" delay="75" aria-label="Increment minutes">
                            <span class="lw-arrow lw-arrow-up" aria-hidden="true"></span>
                        </lw-repeat-button>
                        <lw-repeat-button initial-delay="0" delay="75" aria-label="Decrement minutes">
                            <span class="lw-arrow lw-arrow-down" aria-hidden="true"></span>
                        </lw-repeat-button>
                    </span>
                </div>
                <div class="lw-footer-component lw-footer-component-ampm" role="presentation">
                    <input type="text" class="lw-am-pm-element" aria-label="AM/PM" />
                </div>
                <div class="lw-footer-component lw-footer-component-today" role="presentation">
                    <div tabindex="-1" class="lw-today-element" role="button" aria-label="Now"></div>
                </div>
            </div>`;
    ...
    render() {
        ...
        that._positionDetection = new LW.Utilities.PositionDetection(that, that.$.dropDownContainer, that.$.content, 'close');
        ...
        switch (propertyName) {
            ...
            case 'theme':
                ...
                if (propertyName === 'animation') {
                    ...
                    if (that._defaultFooterTemplateApplied) {
                        Array.from(that.$.calendarDropDown.$.footer.getElementsByTagName('lw-repeat-button')).forEach(function (button) {
            ...
            case 'value': {
                ...
                if (newValue !== null) {
                    parsedValue = LW.Utilities.DateTime.validateDate(newValue, that._now(), that.formatString);
    ...
    _addTimePickerListener() {
        ...
        that.$.timePickerDropDown.$.listen('change', function (event) {
            const oldContext = that.context,
                ...
                updatedValueConstructorParameters = LW.Utilities.DateTime.getConstructorParameters(that._value !== null ? that._value : that._now());
            ...
            that._validateValue(new (Function.prototype.bind.apply(LW.Utilities.DateTime, updatedValueConstructorParameters)));
    ...
    _calendarButtonClickHandler(event) {
        ...
        if (that.hasRippleAnimation) {
            LW.Utilities.Animation.Ripple.animate(that.$.calendarButton, event.pageX, event.pageY);
        }
    ...
    _calendarDropDownChangeHandler(event) {
        ...
        if (this.$.calendarDropDown.selectedDates.length > 0) {
            const newCalendarValue = event.detail.value[0],
                ...
                updatedValueConstructorParameters = LW.Utilities.DateTime.getConstructorParameters(oldValue);
    ...
    _calendarDropDownClickHandler(event) {
        ...
        if (that.autoClose && event.target.closest('.lw-calendar-cell')) {
    ...
    _changeDropDownDisplayMode() {
        function toggleVisibility(dropDownHeader, calendarDropDown, timePickerDropDown, sync) {
            that.$dropDownHeader[dropDownHeader]('lw-hidden');
            that.$calendarDropDown[calendarDropDown]('lw-hidden');

            if (that._timePickerInitialized) {
                that.$.timePickerDropDown.$[timePickerDropDown]('lw-hidden');
            }
        ...
        if (that._dropDownDisplayMode === 'default') {
            ...
            that.$selectDate.addClass('lw-selected');
    ...
    _close() {
        ...
        if (!closingEvent.defaultPrevented) {
            ...
            that.$calendarButton.removeClass('lw-calendar-button-pressed');
            that.$.calendarButton.removeAttribute('active');
            that.$dropDownContainer.addClass('lw-visibility-hidden');

            const hoveredCalendarCell = that.$.calendarDropDown.$.container.querySelector('.lw-calendar-cell[hover]');
    ...
    _detectDisplayMode() {
        ...
        if (that.dropDownDisplayMode !== 'auto') {
            ...
            that._dropDownDisplayMode = LW.Utilities.DateTime.detectDisplayMode(that.value || that.min, that.formatString, that._formatStringRegExp);
        }
    ...
    _dropDownHeaderClickHandler(event) {
        ...
        if (event.target === that.$.selectDate) {
            if (that.$selectDate.hasClass('lw-selected')) {
                return;
            }

            that.$selectDate.addClass('lw-selected');
            that.$selectTime.removeClass('lw-selected');
            that.$.timePickerDropDown.$.addClass('lw-hidden');
            that.$calendarDropDown.removeClass('lw-hidden');
        }
        else {
            if (that.$selectTime.hasClass('lw-selected')) {
                return;
            }

            that.$selectDate.removeClass('lw-selected');
            that.$selectTime.addClass('lw-selected');
            that.$calendarDropDown.addClass('lw-hidden');

            if (that._timePickerInitialized) {
                that.$.timePickerDropDown.$.removeClass('lw-hidden');
            }
            else {
                that._initializeTimePicker();
            }
    ...
    _dropDownKeydownHandler(event) {
        ...
        else if ((key === 'Enter' || key === ' ') && activeElement.classList.contains('lw-footer-component-today')) {
            event.preventDefault();
            that._validateValue(that._now());
            that.close();
        }
        else if (key === 'Enter' &&
            (activeElement === that.$.calendarDropDown.$.body && that.$.calendarDropDown.querySelectorAll('.lw-calendar-cell[selected][focus]').length > 0 ||
    ...
    _dropDownTransitionendHandler() {
        ...
        if (that.opened && that._toFocus) {
            ...
            if (that._timePickerInitialized &&
                !that.$.timePickerDropDown.classList.contains('lw-hidden')) {
    ...
    _footerChangeHandler(event) {
        ...
        if (target.classList.contains('lw-hour-element')) {
        ...
        else if (target.classList.contains('lw-minute-element')) {
            value = parseInt(value, 10);
        ...
        const newValueConstructorParameters = LW.Utilities.DateTime.getConstructorParameters(dateTimePickerValue);

        newValueConstructorParameters[that._codeToIndex[timePartCode]] = value;
        newValueConstructorParameters.unshift(null);

        that._validateValue(new (Function.prototype.bind.apply(LW.Utilities.DateTime, newValueConstructorParameters)));
    ...
    _footerClickHandler(event) {
        ...
        if (target.classList.contains('lw-today-element')) {
            // "Now" icon is clicked
            that._validateValue(that._now());
            ...
            const closestRepeatButton = target.closest('lw-repeat-button');

            if (closestRepeatButton !== null) {
                const decrement = closestRepeatButton === closestRepeatButton.parentElement.children[1],
                    timePartCode = closestRepeatButton.closest('.lw-footer-component').classList.contains('lw-footer-component-hour') ?
                        'hh' : 'mm';
    ...
    _handleManualTimePartEdit(newTimePartValue) {
        const that = this,
            newValueConstructorParameters = LW.Utilities.DateTime.getConstructorParameters(that._value);
    ...
    _initializeTimePicker() {
        ...
        const timePickerDropDown = document.createElement('lw-time-picker');
        ...
        timePickerDropDown.$ = LW.Utilities.Extend(timePickerDropDown);
    ...
    _inputFocusHandler() {
        ...
        if (that.editMode !== 'full') {
            const timeout = LW.Utilities.Core.isMobile ? 10 : 0;
    ...
    _inputDownHandler() {
        ...
        if (that._fullEdit || that._highlightedTimePartEdit || that.editMode === 'full' || LW.Utilities.Core.isMobile) {
            return;
        }
    ...
    _now() {
        const that = this,
            now = new LW.Utilities.DateTime('today', 'Local');
    ...
    _open(initial) {
        ...
        that.$calendarButton.addClass('lw-calendar-button-pressed');
        that.$.calendarButton.setAttribute('active', '');

        if (that._dropDownDisplayMode !== 'default' && that._dropDownDisplayMode !== 'timePicker' ||
            !that._timePickerInitialized || (that._timePickerInitialized && that.$.timePickerDropDown.$.hasClass('lw-hidden'))) {
            that.$calendarDropDown.removeClass('lw-hidden');
        }
    ...
    _setDefaultFooterTemplate(initialization) {
        ...
        that._hourElement = that.$.calendarDropDown.getElementsByClassName('lw-hour-element')[0];
        that._minuteElement = that.$.calendarDropDown.getElementsByClassName('lw-minute-element')[0];
        that._ampmElement = that.$.calendarDropDown.getElementsByClassName('lw-am-pm-element')[0];
        that._todayElement = that.$.calendarDropDown.getElementsByClassName('lw-today-element')[0];
    ...
    _setFocusable() {
        ...
        if (that._defaultFooterTemplateApplied) {
            ...
            that.$.calendarDropDown.getElementsByClassName('lw-footer-component-today')[0].tabIndex = index;
        }
    }
    ...
    _validateInitialPropertyValues() {
        ...
        if (that.value !== null) {
            parsedValue = LW.Utilities.DateTime.validateDate(that.value, new LW.Utilities.DateTime(), that.formatString);
        ...
        that._validateInterval(new LW.Utilities.TimeSpan(0, 0, 1));
    ...
    _validateInterval(referenceValue) {
        ...
        if (interval instanceof LW.Utilities.TimeSpan) {
            validValue = interval;
        }
            ...
            if (indexOfTimeSpan !== -1 && indexOfBracket !== -1) {
                ...
                if (new RegExp(/(^(\d+)(,\s*\d+)*$)/g).test(validValue)) {
                    ...
                    validValue = new (Function.prototype.bind.apply(LW.Utilities.TimeSpan, validValue));
                }
            }

            if (validValue instanceof LW.Utilities.TimeSpan === false) {
                if (validValue.trim() === '') {
                ...
                try {
                    validValue = new LW.Utilities.TimeSpan(validValue);
                }
                catch (error) {
                    that.interval = referenceValue;
                    return;
            ...
            validValue = new LW.Utilities.TimeSpan(interval);
        }
        else {
            that.interval = referenceValue;
            return;
    ...
    _validateMinMax(which, referenceValue) {
        ...
        if (which !== 'max') {
            that.min = LW.Utilities.DateTime.validateDate(that.min, referenceValue || new LW.Utilities.DateTime(1600, 1, 1), that.formatString);
            that.min = that.min.toTimeZone(that._outputTimeZone);
            minChanged = true;
        }

        if (which !== 'min') {
            that.max = LW.Utilities.DateTime.validateDate(that.max, referenceValue || new LW.Utilities.DateTime(3001, 1, 1), that.formatString);
            that.max = that.max.toTimeZone(that._outputTimeZone);
    ...
    _validateRestrictedDates() {
        ...
        for (let i = 0; i < that.restrictedDates.length; i++) {
            ...
            let validRestrictedDate = LW.Utilities.DateTime.validateDate(currentRestrictedDate, 'invalid', that.formatString);
    ...
    _validateValue(value, referenceValue, fireEvent, programmatic) {
        ...
        if (value !== undefined) {
            that._value = LW.Utilities.DateTime.validateDate(value, oldValue, that.formatString);
        }
