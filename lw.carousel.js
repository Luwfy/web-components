...
/**
* lwCarousel custom element.
*/
LW('lw-carousel', class Carousel extends LW.ContentElement {
    /**
    ...
    */
    template() {
        return `<div id="container" role="presentation">
                    <div id="itemsContainer" inner-h-t-m-l="[[innerHTML]]" class="lw-items-container" role="presentation"><content></content></div>
                    <lw-repeat-button initial-delay="0" right-to-left="[[rightToLeft]]" delay="[[delay]]" id="arrowLeft" animation="[[animation]]" unfocusable class="lw-arrow lw-arrow-left" aria-label="Previous slide"></lw-repeat-button>
                    <lw-repeat-button initial-delay="0" right-to-left="[[rightToLeft]]" delay="[[delay]]" id="arrowRight" animation="[[animation]]" unfocusable class="lw-arrow lw-arrow-right" aria-label="Next slide"></lw-repeat-button>
                    <div id="indicatorsContainer" class="lw-indicators-container" role="tablist"></div>
                </div>`;
    }
    ...
    */
    _animationTrigger(skip) {
        ...
        for (let i = 0; i < that._items.length; i++) {
            ...
            that._items[i].classList.add('lw-animate-trigger');
        }
    }
    ...
    */
    _handleIndicatorsState(from, to) {
        ...
        if (from !== to) {
            that._indicators[from].classList.remove('lw-active');
            that._indicators[from].setAttribute('aria-selected', false);
        }

        that._indicators[to].classList.add('lw-active');
    ...
    */
    _handleItemClick(event) {
        const that = this,
            clickedItem = event.target.closest('.lw-carousel-item');
    ...
    */
    _handleItemsState(from, to) {
        ...
        if (from !== to) {
            that._items[from].classList.remove('lw-active');
            that._items[from].classList.add('lw-out');
        }

        that._items[to].classList.add('lw-active');
    }
    ...
    */
    _handleIndicatorsContainerClick(event) {
        const that = this,
            clickedItem = event.target.closest('.lw-indicator'),
            previousIndex = that._currentIndex;
    ...
    */
    _generateIndicator(id) {
        ...
        indicatorContainer.classList.add('lw-indicator');
    ...
    _generateItem(id) {
        ...
        if (that.itemTemplate) {
            ...
            if (typeof item === 'string') {
                ...
                itemContainer.innerHTML = `<div class="lw-carousel-item-container" style="background-image:url('${item.image || ''}')" role="presentation">
                        <h2 id="${itemContainer.id + 'Label'}" class="lw-carousel-item-label">${item.label || ''}</h2 >
                        <p class="lw-carousel-item-content">${item.content || ''}</p>
                    </div>
                    <div class="lw-carousel-html-content">${item.HTMLcontent || ''}</div>
                </div>`;
            }
        }
        ...
        itemContainer.classList.add('lw-carousel-item');
    ...
    */
    _handleTransitionEnd(event) {
        if (!event.target.classList.contains('lw-carousel-item') || event.target.getAttribute('position')) {
            return;
        }
    ...
    */
    _removeFadeOut() {
        const that = this,
            fadeOutItems = that.$.itemsContainer.getElementsByClassName('lw-out');

        if (fadeOutItems.length) {
            for (let i = 0; i < fadeOutItems.length; i++) {
                fadeOutItems[i].classList.remove('lw-out');
            }
        }
    }
});