...
class DataAdapter {
    constructor ( config ) {
        ...
        that.$document = LW.Utilities.Extend( document );
        that.boundSource = false === that.observable ? [] : new LW.ObservableArray();
        that.dataItemById = [];
        ...
        Object.defineProperty( that, 'groupBy', {
            ...
            set( value ) {
                ...
                that._groupBy = new LW.ObservableArray( value );
                that._groupBy.notify( function () {
                    updateGrouping();
                } );
    ...
    update( index, dataSourceItem ) {
        ...
        if ( LW.Utilities.Types.isArray( index ) && LW.Utilities.Types.isArray( dataSourceItem ) ) {
    ...
    clear() {
        ...
        that._cachedValues = [];
        that.boundSource = that.observable ? new LW.ObservableArray() : [];
        that.dataItemById = [];
        that.refreshHierarchy();
    }
    ...
    _bindToArray() {
        ...
        that.boundSource = false === that.observable ? dataArray : new LW.ObservableArray( dataArray );
    ...
    _createFilter( dataType, filterExpressions ) {
        ...
        for ( let i = 0; i < filterExpressionsArray.length; i++ ) {
            ...
            if ( filterExpression.length > 1 ) {
                const filterSubGroup = new LW.FilterGroup();
    ...
    _filter( filters, operator = 'and' ) {
        ...
        for ( let i = 0; i < filters.length; i++ ) {
            ...
            if ( filter[ 1 ] instanceof LW.FilterGroup ) {
    ...
    _sort( dataSource, sortColumns, directions, dataTypes, customSortingCallback ) {
        ...
        if ( dataSource && dataSource.constructor && dataSource instanceof LW.ObservableArray ) {
        ...
        const getCompareFunction = function ( a, knownDataType ) {
            ...
            switch ( dataType ) {
                ...
                case 'dateTime':
                    ...
                    else if ( a instanceof LW.Utilities.DateTime ||
                        a instanceof LW.Utilities.BigNumber ) {
                        compareFunction = function ( a, b ) {
                            return a.compare( b );
                        };
                    }
                ...
                case 'object':
                    ...
                    else if ( a instanceof LW.Utilities.Complex || ( window.NIComplex && a instanceof window.NIComplex ) ) {
                        const complexNumericProcessor = new LW.Utilities.ComplexNumericProcessor();
    ...
    filter( filterColumns, filterGroups, customFilteringCallback ) {
        LW.DataAdapter.Filter( this.boundSource, filterColumns, filterGroups, customFilteringCallback );
    }

    sort( sortColumns, directions, customSortingCallback ) {
        LW.DataAdapter.Sort( this.boundSource, sortColumns, directions, customSortingCallback );
    }
...
LW.DataAdapter = window.lwDataAdapter = DataAdapter;
