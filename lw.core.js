		...
		for (let i = 0; i < htmlElement.attributes.length; i++) {
			...
			if (attributeName.indexOf('(') >= 0) {
				...
				if (that.ownerElement && !that.ownerElement.dataContext) {
					that.ownerElement.templateListeners[htmlElement.getAttribute('lw-id') + '.' + eventName] = attributeValue;
		...
		while (lastIndex < length) {
			...
			/*   if (twoWay) {
				   ...
				   if (pathString.indexOf('::') >= 0) {
					   ...
					   that.ownerElement['$' + node.getAttribute('lw-id')].listen(eventName, function () {
				   ...
				   if (node.$ && node.$.isCustomElement) {
					   ...
					   that.ownerElement['$' + node.getAttribute('lw-id')].listen(attributeName + '-changed', function (event) {
...
export class App {
	constructor(object) {
		...
		that._appRoot.classList.add('lw-visibility-hidden');
		...
		const init = function () {
			...
			that._appRoot.classList.remove('lw-visibility-hidden');
		}
	...
	_addAttributeBindings(ownerElement, ownerPropertyName) {
		...
		const traverseBindings = function (bindings) {
			...
			for (let index in bindings.children) {
				...
				if (child.node && child.node.getAttribute && child.node.getAttribute('lw-for') && child.node.getAttribute('lw-for-rendered') === null) {
					continue;
				}
	...
	_addModelBindings(ownerElement) {
		...
		const elements = ownerElement ? ownerElement.querySelectorAll('[lw-model]') : that._appRoot.querySelectorAll('[lw-model]');

		for (let i = 0; i < elements.length; i++) {
			const element = elements[i];
			let bind = {
				value: element.getAttribute('lw-model')
			};

			if (element.type === 'list') {
				bind = {
					selectedValues: element.getAttribute('lw-model')
				};
			}

			if (element.type === 'checkbox' || element.type === 'radio') {
				bind = {
					checked: element.getAttribute('lw-model')
				};
			}
	...
	_renderForBindings(invokeRender) {
		...
		const elements = that._appRoot.querySelectorAll('[lw-for]');

		for (let i = 0; i < elements.length; i++) {
			const element = elements[i];
			const dataName = element.getAttribute('lw-for');
			...
			element.setAttribute('lw-for-rendered', '');

			const selfRepeat = element.getAttribute('lw-for-self') !== null;
			const toRemove = !selfRepeat ? element.querySelectorAll('.lw-for-item-' + element.id) : element.parentNode.querySelectorAll('.lw-for-item-' + element.id);
			...
			for (let j = 0; j < array.length; j++) {
				...
				clonedNode.classList.add('lw-for-item');
				clonedNode.classList.add('lw-for-item-' + element.id);

				clonedNode.removeAttribute('lw-for');
			...
			let children = that._appRoot.querySelectorAll('.lw-for-item-' + element.id);
