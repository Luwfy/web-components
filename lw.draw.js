...
// Draw class
LW.Utilities.Assign('Draw', class Draw {
    constructor(host, renderEngine) {
        ...
        for (let i in functions) {
            that._addFn(LW.Utilities.Draw.prototype, functions[i]);
        }
    }
    ...
    createRenderer(widgetInstance, host) {
        ...
        if (document.createElementNS && self.renderEngine !== 'HTML5') {
            renderer = new LW.Utilities.SvgRenderer(this);
        }

        if (renderer === null && (self.renderEngine === 'HTML5' || self.renderEngine === undefined)) {
            renderer = new LW.Utilities.HTML5Renderer(this);
        }
            ...
            if (canvas) {
                if (isPDF) {
                    ...
                    if (!LW.Utilities.DataExporter) {
                        widgetInstance.error(widgetInstance.localize('missingReference', { files: 'lw.export.js' }));
                    }
...
// SvgRenderer class
LW.Utilities.Assign('Renderer', class Renderer {
    constructor(draw) {
        const that = this;
...
// SvgRenderer class
LW.Utilities.Assign('SvgRenderer', class SvgRenderer extends LW.Utilities.Renderer {
    constructor(draw) {
        super(draw);
...
// HTML5Renderer class
LW.Utilities.Assign('HTML5Renderer', class HTML5Renderer extends LW.Utilities.Renderer {
    constructor(draw) {
        super(draw);

        const that = this;

        that._renderers = new LW.Utilities.HTML5RenderHelpers(that);
    }
...
LW.Utilities.Assign('HTML5RenderHelpers', class HTML5RenderHelpers {
...
// Plot class
LW.Utilities.Assign('Plot', class Plot {
