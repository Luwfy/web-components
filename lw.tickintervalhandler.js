...
LW.Utilities.Assign('TickIntervalHandler', class TickIntervalHandler {
    ...
    getNiceIntervalInteger(min, max, divisionCountEstimate, majorInterval) {
        const rangeDelta = new LW.Utilities.BigNumber(max).subtract(new LW.Utilities.BigNumber(min)),
            exponent = Math.floor(Math.log10(rangeDelta.toString()) - Math.log10(divisionCountEstimate)),
            nearestPowerOfTen = new LW.Utilities.BigNumber(10).pow(new LW.Utilities.BigNumber(exponent)),
            factor = new LW.Utilities.BigNumber(divisionCountEstimate).multiply(nearestPowerOfTen);
        let niceFactor;
        if (rangeDelta.compare(new LW.Utilities.BigNumber(2 * factor)) === -1) {
            niceFactor = 1;
        }
        else if (rangeDelta.compare(new LW.Utilities.BigNumber(3 * factor)) === -1) {
            niceFactor = 2;
        }
        else if (rangeDelta.compare(new LW.Utilities.BigNumber(7 * factor)) === -1) {
            niceFactor = 5;
        }
        else {
            niceFactor = 10;
        }
        let niceInterval = new LW.Utilities.BigNumber(niceFactor).multiply(nearestPowerOfTen);

        if (majorInterval && new LW.Utilities.BigNumber(this.context._range).divide(niceInterval).compare(divisionCountEstimate) === 1) {
            switch (niceFactor) {
                case 5:
                    niceFactor = 10;
                    break;
            ...
            niceInterval = new LW.Utilities.BigNumber(niceFactor).multiply(nearestPowerOfTen);
        }

        if (niceInterval.compare(1) === -1) {
            niceInterval = new LW.Utilities.BigNumber(1);
        }
    ...
    getPossibleBiggerLabelInteger(divisionCountEstimate, majorInterval) {
        ...
        let secondValue = new LW.Utilities.BigNumber(context.min).subtract(new LW.Utilities.BigNumber(context.min).mod(majorInterval)).add(majorInterval),
            currentDrawValue = secondValue,
            largestLabel, currentLabel;
