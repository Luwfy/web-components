...
LW('bootstrap-button', class Button extends LW.ContentElement {
	// Button's properties.
...
LW('bootstrap-check-box', class CheckBox extends LW.ContentElement {
	// Button's properties.
	static get properties() {
		return {
			'checked': {
	...
	*/
	...
	ready() {
		...
		if (!(that instanceof LW.Bootstrap.ToggleButton)) {
	...
	toggle(event) {
		...
		if (that.checked) {
			...
			if (that instanceof LW.Bootstrap.RadioButton) {
...
LW('bootstrap-switch-button', class SwitchButton extends LW.Bootstrap.CheckBox {
	// Button's properties.
	static get properties() {
		return {
			'type': {
...
LW('bootstrap-toggle-button', class ToggleButton extends LW.Bootstrap.CheckBox {
	// Button's properties.
	static get properties() {
		return {
			'styleMode': {
...
LW('bootstrap-radio-button', class RadioButton extends LW.Bootstrap.CheckBox {
	// Button's properties.
	static get properties() {
		return {
			'styleMode': {
...
LW('bootstrap-drop-down', class DropDown extends LW.ContentElement {
	...
	*/
	...
	propertyChangedHandler(propertyName, oldValue, newValue) {
		...
		if (propertyName === 'styleMode' || propertyName === 'sizeMode') {
			...
			that.$.button = that.querySelector('[lw-id="button"]');
	...
	ready() {
		...
		that._positionDetection = new LW.Utilities.PositionDetection(that, that.$.dropDownContainer, that.$.container, 'hide');
	...
	_customPositionDropDown() {
		const that = this,
			coordinates = that.$[that instanceof LW.Bootstrap.SplitButton ? 'button' : 'container'].getBoundingClientRect(),
...
LW('bootstrap-split-button', class SplitButton extends LW.Bootstrap.DropDown {
...
LW('bootstrap-input-group', class InputGroup extends LW.ContentElement {
...
LW('bootstrap-modal', class Modal extends LW.ContentElement {
	// Element's properties.
	static get properties() {
		return {
			'sizeMode': {
...
LW('bootstrap-tabs', class Tabs extends LW.ContentElement {
	// Element's properties.
	static get properties() {
		return {
			'sizeMode': {
	...
	*/
	...
	render() {
		...
		while (container.firstElementChild) {
			...
			if (navLink) {
				if (navLink instanceof LW.Bootstrap.DropDown) {
...
LW('bootstrap-range', class Range extends LW.BaseElement {
	// Button's properties.
	static get properties() {
		return {
			'min': {
...
LW('bootstrap-progress-bar', class ProgressBar extends LW.ContentElement {
	// Button's properties.
	static get properties() {
		return {
			'min': {
...
LW('bootstrap-circular', class Circular extends LW.Bootstrap.ProgressBar {
	static get properties() {
			return {
				'type': {
					value: 'circular',
...
LW('bootstrap-input', class TextBox extends LW.ContentElement {
	static get properties() {
		return {
			'name': {
				value: '',
...
LW('bootstrap-textarea', class TextArea extends LW.Bootstrap.Input {
	template() {
		return `<div><label input id="label">
...
LW('bootstrap-file-input', class FileInput extends LW.ContentElement {
	static get properties() {
		return {
			'name': {
				value: '',
