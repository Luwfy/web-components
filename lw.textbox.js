...
*/
LW('lw-text-box', class TextBox extends LW.ComboBox {
    /**
    ...
    template() {
        return `<div id="container" role="presentation">
                    ...
                    <div id="content" class="lw-content" role="presentation">
                        <input class="lw-input" type="text" id="input"
                            disabled="[[disabled]]"
                            maxlength="[[maxLength]]"
                            name="[[name]]"
 ...
 */
    _createElement() {
        ...
        that._browserIsIEorEdge = LW.Utilities.Core.Browser.IE || LW.Utilities.Core.Browser.Edge;
    ...
    _focusHandler(event) {
        ...
        if (event.type === 'blur') {
            ...
            if ((that.autoComplete === 'auto' || that.autoComplete === 'inline') && that.$.input.value.length > 0 &&
                ...
                const newValue = that.$.listBox._focusedItem[that.inputMember];

                that.$.input.value = that.displayMode === 'escaped' ? that._toEscapedDisplayMode(newValue) : that._toDefaultDisplayMode(newValue);
                that.set('value', that._toDefaultDisplayMode(newValue));
            }
        ...
        if (false === that._canFireFocus) {
            return;
        }

        that._canFireFocus = false;
        that.$.fireEvent('focus');
        that._canFireFocus = true;
    }
                ...
                if (that.enterKeyBehavior !== 'default') {
                    ...
                    if (that._oldValue !== value) {
                        ...
                        if (that.enterKeyBehavior === 'clearOnSubmit') {
                            if (LW.MaskedTextBox && (that instanceof LW.MaskedTextBox)) {
                                that._cleanMask();
   ...
   */
    ...
    _documentDownHandler(event) {
        ...
        if (that.enableShadowDOM) {
            ...
            if (that._dropDownParent === null) {
                ...
                if (rootElement && rootElement.closest('.lw-drop-down-container') === that.$.dropDownContainer) {
        ...
        const listItem = event.originalEvent.target.closest('lw-list-item');
    ...
    */
    _documentUpHandler(event) {
        ...
        while (target) {
            if (target instanceof LW.ListItem && target.ownerListBox === that.$.listBox) {
                ...
                if (that.displayMode === 'escaped') {
                    ...
                    that.$.input.value = that._toDefaultDisplayMode(target[that.inputMember]);
                }
