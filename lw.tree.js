 ...
 */
LW('lw-tree-item', class TreeItem extends LW.MenuItem {
        ...
        switch (propertyName) {
            ...
            case 'shortcut': {
                ...
                if (!shortcutElement) {
                    ...
                    shortcutElement.className = 'lw-tree-item-shortcut';
 ...
 */
LW('lw-tree-items-group', class TreeItemsGroup extends LW.MenuItemsGroup {
 ...
 * Tree custom element.
 */
LW('lw-tree', class Tree extends LW.Menu {
    // Tree's properties.
    static get properties() {
        return {
            'allowDrag': {
            ...
            'messages': {
                value: {
                    'en': {
                        'noId': 'lw-tree: Saving and loading the element\'s state are not available if the element has no id.'
                    }
                },
                type: 'object',
                extend: true
    ...
    */
    ...
    template() {
        return `<div id="container" role="presentation">
                    <div class="lw-tree-filter-input-container" role="presentation"><input id="filterInput" class="lw-filter-input" disabled="[[disabled]]" placeholder="[[filterInputPlaceholder]]" type="text" role="searchbox" aria-label="[[filterInputPlaceholder]]" /></div>
                    <lw-repeat-button id="scrollButtonNear" class="lw-tree-scroll-button lw-spin-button lw-scroll-button-near lw-hidden" animation="[[animation]]" unfocusable aria-label="Scroll up">
                        <div id="arrowNear" class="lw-arrow lw-arrow-up"></div>
                    </lw-repeat-button>
                    <lw-scroll-viewer id="scrollViewer" animation="[[animation]]" horizontal-scroll-bar-visibility="hidden" right-to-left="[[rightToLeft]]">
                        <div id="mainContainer" class="lw-tree-main-container" role="presentation">
                            <content></content>
                        </div>
                        <input type="text" id="editInput" class="lw-tree-edit-input lw-hidden" />
                    </lw-scroll-viewer>
                    <lw-repeat-button id="scrollButtonFar" class="lw-tree-scroll-button lw-spin-button lw-scroll-button-far lw-hidden" animation="[[animation]]" unfocusable aria-label="Scroll down">
    ...
    detached() {
        ...
        const targetTree = LW.Tree.hoveredTree,
            targetItem = LW.Tree.hoveredItem;

        delete that._dragDetails;
        delete LW.Tree.treeItemDragged;
        delete LW.Tree.hoveredTree;
        delete LW.Tree.hoveredItem;
     ...
     *
     * @param {HTMLElement} item A lw-tree-item/lw-tree-items-group to add to the Tree.
     * @param {HTMLElement/String} sibling The lw-tree-item/lw-tree-items-group (or its id or numeric path) to add the item after.
     */
     ...
     *
     * @param {HTMLElement} item A lw-tree-item/lw-tree-items-group to add to the Tree.
     * @param {HTMLElement/String} sibling The lw-tree-item/lw-tree-items-group (or its id or numeric path) to add the item before.
     */
     ...
     *
     * @param {HTMLElement} item A lw-tree-item/lw-tree-items-group to add to the Tree.
     * @param {HTMLElement/String} parent Optional The lw-tree-items-group (or its id or numeric path) to add the item to.
     */
    addTo(item, parent) {
        const that = this;
        ...
        if (parent === undefined) {
            ...
            if (parent instanceof LW.TreeItemsGroup === false) {
                return;
            }
        ...
        if (item instanceof LW.TreeItemsGroup) {
     ...
     *
     * @param {HTMLElement/String} item lw-tree-item/lw-tree-items-group (or its id or numeric path).
     */
    ensureVisible(item) {
        const that = this;
    ...
    expandAll(animation) {
        const that = this,
            ...
            restoreAnimation = animation === false && that.hasAnimation;
        let level = 1,
            treeItemsGroups = (that.enableShadowDOM ? that.shadowRoot : that).querySelectorAll('lw-tree-items-group[level="1"]');
        ...
        while (treeItemsGroups.length > 0) {
            ...
            level++;
            treeItemsGroups = (that.enableShadowDOM ? that.shadowRoot : that).querySelectorAll('lw-tree-items-group[level="' + level + '"]');
        }
    ...
    expandItem(item, animation) {
        ...
        if (item === undefined || item instanceof LW.TreeItemsGroup === false ||
     ...
     * 
     * @param {HTMLElement/Object} item A lw-tree-item/lw-tree-items-group (or an Object to create an item from) to add to the Tree.
    ...
    insert(item, path) {
        ...
        function createTreeItem(item) {
            const subItems = item[that.itemsMember],
                tagName = item.tagName === 'lw-tree-items-group' || Array.isArray(subItems) ? 'lw-tree-items-group' : 'lw-tree-item',
            ...
            if (tagName === 'lw-tree-items-group') {
                if (item.expanded) {
        ...
        else if (path instanceof LW.TreeItem || path instanceof LW.TreeItemsGroup) {
        ...
        if (pathParts.length === 1) {
            ...
            else if (parent && parent instanceof LW.TreeItemsGroup) {
    ...
    loadState(state) {
        ...
        if (!state) {
            ...
            state = window.localStorage.getItem('lwTree' + that.id);
     ...
     *
     * @param {HTMLElement/String} item The lw-tree-item/lw-tree-items-group (or its id or numeric path) to remove.
     */
    moveDown(item) {
        const that = this;
    ...
    saveState() {
        ...
        if (!that.id) {
            ...
            window.localStorage.setItem('lwTree' + that.id, JSON.stringify(that._state));
        }
    ...
    updateItem(item, newItem) {
        ...
        const treeItem = item instanceof LW.TreeItem;
    ...
    _addDragFeedback() {
        ...
        feedback.className = 'lw-tree-item-feedback';
    ...
    _applyFilter(filterQuery) {
        ...
        function filter(item, childItems) {
            ...
            for (let i = 0; i < childItems.length; i++) {
                ...
                if (currentChild instanceof LW.TreeItem) {
    ...
    _applyHierarchicalSelection(item, validatedSelection) {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (currentChild instanceof LW.TreeItemsGroup) {
    ...
    _applySelection(combine, oldSelectedIndexes) {
        ...
        const selectionMode = that.selectionMode,
            itemsWithSelectedAttr = Array.from((that.shadowRoot || that).querySelectorAll('lw-tree-item[selected], lw-tree-items-group[selected]'));
    ...
    _createElement() {
        ...
        const items = (that.shadowRoot || that).querySelectorAll('lw-tree-item, lw-tree-items-group');

        const itemsReady = function () {
            let previousState;

            if (that.autoLoadState) {
                previousState = window.localStorage.getItem('lwTree' + that.id);
    ...
    _dblclickHandler(pressedItem, arrow, event) {
        ...
        if (that.toggleMode !== 'dblclick' || pressedItem instanceof LW.TreeItem || arrow) {
            return;
        }
    ...
    _discardKeyboardHover(overrideCheck, mouseoutMouseover) {
        ...
        if (!overrideCheck &&
            that._hoveredViaKeyboard instanceof LW.MenuItemsGroup &&
    ...
    _documentUpHandler(event) {
        ...
        if (downItem) {
            const pressedItem = upTarget.closest('lw-tree-item') || upTarget.closest('lw-tree-items-group');

            if (pressedItem && pressedItem.selected && pressedItem === downItem &&
                (!that._dragDetails || !that._dragDetails.FeedbackShown) &&
                !(pressedItem instanceof LW.TreeItemsGroup && (upTarget.closest('.lw-tree-items-group-arrow') ||
    ...
    _keydownHandler(event) {
        ...
        const treeItems = Array.from(that.$.mainContainer.querySelectorAll('lw-tree-item, lw-tree-items-group')),
        ...
        switch (key) {
            ...
            case 'ArrowLeft':
                ...
                if (highlightedItem instanceof LW.TreeItem) {
                ...
                break;
            case 'ArrowRight':
                if (!highlightedItem || highlightedItem instanceof LW.TreeItem) {
                    return;
                }
            ...
            case 'End':
                ...
                break;
            case 'Enter':
                if (highlightedItem && highlightedItem instanceof LW.TreeItemsGroup) {
                    that._menuItemsGroupSelectionHandler(highlightedItem, { target: highlightedItem, type: 'keydown' });
                }
    ...
    _menuItemsGroupSelectionHandler(closestItemsGroup, event, fireEvent) {
        const that = this,
            ...
            arrow = event.originalEvent ? event.originalEvent.target.classList.contains('lw-tree-items-group-arrow') : false,
        ...
        if (that._isContainerOpened(level, container)) {
            ...
            closestItemsGroup.$.addClass('lw-tree-items-group-opened');
            closestItemsGroup.$.addClass('lw-tree-items-group-expanded');
    ...
    _mouseenterHandler() {
        ...
        if (LW.Tree.treeItemDragged && that.allowDrop && !that.disabled) {
            LW.Tree.hoveredTree = that;
        }
    }
    ...
    _mouseleaveHandler() {
        ...
        if (LW.Tree.treeItemDragged) {
            if (LW.Tree.hoveredTree) {
                clearInterval(LW.Tree.hoveredTree._dragInterval);
                delete LW.Tree.hoveredTree;
            }

            const hoveredItem = LW.Tree.hoveredItem;
    ...
    _moveHandler(event) {
        ...
        if (that._isMobile) {
            const oldHoveredItem = LW.Tree.hoveredItem;
            ...
            if (realTarget) {
                hoveredTree = realTarget.closest('lw-tree');

                if (hoveredTree && hoveredTree.allowDrop) {
                    LW.Tree.hoveredTree = hoveredTree;
                    target = realTarget.closest('lw-tree-item') || realTarget.closest('lw-tree-items-group');
        ...
        hoveredTree = LW.Tree.hoveredTree;
        ...
        if (!that._isMobile) {
            ...
            if (target && target.closest) {
                hoveredItem = target.closest('lw-tree-item') || target.closest('lw-tree-items-group');
            }
        }

        if (hoveredItem) {
            if (LW.Tree.hoveredItem && hoveredItem !== LW.Tree.hoveredItem) {
                LW.Tree.hoveredItem.classList.remove('drop-target');
                LW.Tree.hoveredItem.classList.remove('top');
                LW.Tree.hoveredItem.classList.remove('bottom');
            }

            LW.Tree.hoveredItem = hoveredItem;
            ...
            if (hoveredItem instanceof LW.TreeItem) {
            ...
            if (target === hoveredTree.$.scrollViewer.$.scrollViewerContainer) {
                if (hoveredTree._menuItems['0']) {
                    ...
                    if (lastVisibleItem) {
                        LW.Tree.hoveredItem = lastVisibleItem;
                        LW.Tree.hoveredItem.classList.add('bottom');
                    }
                    else {
                        LW.Tree.hoveredItem = hoveredTree.$.container;
                    }

                    LW.Tree.hoveredItem.classList.add('drop-target');
                }
                else {
                    LW.Tree.hoveredItem = hoveredTree.$.container;
                    LW.Tree.hoveredItem.classList.add('drop-target');
    ...
    /**
     * Moves the sub-items of a lw-tree-items-group.
     */
    ...
    _continueSelection(target, event) {
        const that = this,
            pressedItem = target.closest('lw-tree-item') || target.closest('lw-tree-items-group');

        if (!(pressedItem && pressedItem.parentElement && that._canItemBeSelected(pressedItem))) {
            return;
        }

        const arrow = target.closest('.lw-tree-items-group-arrow');
        let selectionHandler;

        if (pressedItem instanceof LW.TreeItem) {
    ...
    _selectItemRange(from, to) {
        const that = this,
            treeItems = Array.from(that.$.mainContainer.querySelectorAll('lw-tree-item, lw-tree-items-group')),
    ...
    _startDragging(pressedItem, event) {
        ...
        LW.Tree.treeItemDragged = true;
        that.$.scrollViewer._scrollView.disableSwipeScroll = true;

        if (that.allowDrop) {
            LW.Tree.hoveredTree = that;
            LW.Tree.hoveredItem = pressedItem;
        }
    }
