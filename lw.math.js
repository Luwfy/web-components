...
// BigNumber class in LW.Utilities
LW.Utilities.Assign('BigNumber', class UtilitiesBigNumber {
    ...
    constructor(n, p, r) {
        ...
        if (n instanceof LW.Utilities.BigNumber) {
            if (typeof n._d === 'bigint') {
                return new LW.Utilities.BigNumber(n._d.toString());
            }
        ...
        o.precision = isNaN(p = Math.abs(p)) ? LW.Utilities.BigNumber.defaultPrecision : p;
        o.roundType = isNaN(r = Math.abs(r)) ? LW.Utilities.BigNumber.defaultRoundType : r;
        o._s = (n += '').charAt(0) == '-';
    ...
    static get bigIntSupport() {
        return bigIntNativeSupport && LW.Utilities.BigNumber.ignoreBigIntNativeSupport !== true;
    }

    add(n) {
        if (LW.Utilities.BigNumber.bigIntSupport) {
            return new LW.Utilities.BigNumber(this._d + new LW.Utilities.BigNumber(n)._d);
        }
        ...
        if (n === 0 || (n.constructor === LW.Utilities.BigNumber && n._d.length === 1 && n._d[0] === 0)) {
            return new LW.Utilities.BigNumber(that);
        }

        if (that._s != (n = new LW.Utilities.BigNumber(n))._s)
            return n._s ^= 1, that.subtract(n);
        var o = new LW.Utilities.BigNumber(that), a = o._d, b = n._d, la = o._f,
            lb = n._f, i, r;
        n = Math.max(la, lb);
    ...
    subtract(n) {
        ...
        var o = new LW.Utilities.BigNumber(that), c = o.abs().compare(n.abs()) + 1, a = c ? o : n, b = c ? n : o, la = a._f, lb = b._f, d = la, i, j;
    ...
    multiply(n) {
        ...
        var o = new LW.Utilities.BigNumber(that), r = o._d.length >= (n = new LW.Utilities.BigNumber(n))._d.length, a = (r ? o : n)._d,
            b = (r ? n : o)._d, la = a.length, lb = b.length, x = new LW.Utilities.BigNumber, i, j, s;
        for (i = lb; i; r && s.unshift(r), x.set(x.add(new LW.Utilities.BigNumber(s.join('')))))
    ...
    divide(n) {
        ...
        if ((n = new LW.Utilities.BigNumber(n)) == '0')
            throw new Error('Division by 0');
        else if (that == '0')
            return new LW.Utilities.BigNumber;
        var o = new LW.Utilities.BigNumber(that), a = o._d, b = n._d, la = a.length - o._f,
            lb = b.length - n._f, r = new LW.Utilities.BigNumber, i = 0, j, s, l, f = 1, c = 0, e = 0;
        r._s = o._s != n._s, r.precision = Math.max(o.precision, n.precision),
            r._f = +r._d.pop(), la != lb && o._zeroes(la > lb ? b : a, Math.abs(la - lb));
        n._f = b.length, b = n, b._s = false, b = b.round();
        for (n = new LW.Utilities.BigNumber; a[0] == '0'; a.shift());
        out:
        do {
            ...
            var y;
            for (s = new LW.Utilities.BigNumber, j = 0; n.compare(y = s.add(b)) + 1 && ++j; s.set(y));
    ...
    pow(n) {
        if (LW.Utilities.BigNumber.bigIntSupport) {
            ...
            for (let i = BigInt(0); i < new LW.Utilities.BigNumber(n)._d; i = i + BigInt(1)) {
        ...
        var o = new LW.Utilities.BigNumber(that), i;
        if ((n = (new LW.Utilities.BigNumber(n)).intPart()) == 0) return o.set(1);
        for (i = Math.abs(n); --i; o.set(o.multiply(that)));
        return n < 0 ? o.set((new LW.Utilities.BigNumber(1)).divide(o)) : o;
    }

    set(n) {
        n = new LW.Utilities.BigNumber(n);
        this._d = n._d;
        this._f = n._f;
        this._s = n._s;
        return this;
    ...
    compare(n) {
        if (LW.Utilities.BigNumber.bigIntSupport) {
            const otherNumber = new LW.Utilities.BigNumber(n)._d;
        ...
        var a = that, la = that._f, b = new LW.Utilities.BigNumber(n), lb = b._f, r = [-1, 1], i, l, arr;
    ...
    negate() {
        if (LW.Utilities.BigNumber.bigIntSupport) {
            return new LW.Utilities.BigNumber(this._d * BigInt(-1));
        }

        let that = this.normalizeOperand(this);

        var n = new LW.Utilities.BigNumber(that); return n._s ^= 1, n;
    }

    abs() {
        if (LW.Utilities.BigNumber.bigIntSupport) {
            return new LW.Utilities.BigNumber(this._d < 0 ? this._d * BigInt(-1) : this._d);
        }

        let that = this.normalizeOperand(this);

        var n = new LW.Utilities.BigNumber(that); return n._s = 0, n;
    }

    intPart() {
        if (LW.Utilities.BigNumber.bigIntSupport) {
            return new LW.Utilities.BigNumber(this._d);
        }

        let that = this.normalizeOperand(this);

        return new LW.Utilities.BigNumber((that._s ? '-' : '') + (that._d.slice(0, that._f).join('') || '0'));
    }
    ...
    toString(radix, wordLength, leadingZeros) {
        ...
        function toBinary(positiveNumber) {
            var two = new LW.Utilities.BigNumber(2),
                remainder,
                remaindersArray = [],
                temp;
            ...
            while (temp.compare(new LW.Utilities.BigNumber(0)) === 1);
    ...
    round() {
        ...
        var $ = LW.Utilities.BigNumber, r = this.roundType, b = this._d, d, p, n, x;
