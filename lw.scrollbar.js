...
*/
LW('lw-scroll-bar', class ScrollBar extends LW.BaseElement {
    ...
    template() {
        const template =
            `<div id="container" class="lw-container" role="presentation">
                    <div id="nearButton" class="lw-scroll-button lw-arrow-left" role="presentation" aria-hidden="true"></div>
                    <div  id="track" class="lw-track" role="presentation">
                        <div id="thumb" class="lw-thumb" role="presentation"></div>
                    </div>
                    <div id="farButton" class="lw-scroll-button lw-arrow-right" role="presentation" aria-hidden="true"></div>
            </div>`;
    ...
    _calculateThumbSize(scrollAreaSize) {
        ...
        if (positions >= 1 && enoughSpace) {
            ...
            if (that.$.thumb.className.indexOf('lw-hidden') >= 0) {
                that.$thumb.removeClass('lw-hidden');
            }
        }
        else {
            that.$thumb.addClass('lw-hidden');
        }
    ...
    _layout() {
        ...
        if (that.orientation === 'horizontal') {
            if (that.$.nearButton.classList.contains('lw-arrow-up')) {
                that.$.nearButton.classList.remove('lw-arrow-up');
            }

            if (that.$.farButton.classList.contains('lw-arrow-down')) {
                that.$.farButton.classList.remove('lw-arrow-down');
            }

            if (!that.$.nearButton.classList.contains('lw-arrow-left')) {
                that.$.nearButton.classList.add('lw-arrow-left');
            }

            if (!that.$.farButton.classList.contains('lw-arrow-right')) {
                that.$.farButton.classList.add('lw-arrow-right');
            }
        }
        else {
            if (that.$.nearButton.classList.contains('lw-arrow-left')) {
                that.$.nearButton.classList.remove('lw-arrow-left');
            }

            if (that.$.farButton.classList.contains('lw-arrow-right')) {
                that.$.farButton.classList.remove('lw-arrow-right');
            }

            if (!that.$.nearButton.classList.contains('lw-arrow-up')) {
                that.$.nearButton.classList.add('lw-arrow-up');
            }

            if (!that.$.farButton.classList.contains('lw-arrow-down')) {
                that.$.farButton.classList.add('lw-arrow-down');
            }
        }
